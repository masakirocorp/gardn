...
use support::{
    ...
    unregister_spawned_hako_pid, wait_for_file, wait_for_socket, wait_until,
};
...
fn explicit_detach_message_keeps_server_available_for_new_clients() {
    ...
    let (version, error) =
        client_handshake(&mut reattached, 11, 80, 24).expect("reattach handshake should succeed");
    assert_eq!(version, 11);
    assert!(error.is_none(), "{:?}", error);
