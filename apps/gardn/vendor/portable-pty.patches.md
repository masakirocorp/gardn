...
behavior or exposes an option that preserves Gardn's invariant.

## 0001 force system ConPTY

status: active

patch: `apps/gardn/vendor/patches/portable-pty/0001-force-system-conpty.patch`

vendored base: `portable-pty 0.9.0`

local file:

- `apps/gardn/vendor/portable-pty/src/win/psuedocon.rs`

reason: `portable-pty` 0.9.0 first loads `kernel32.dll`, then probes a bare
`conpty.dll` from the DLL search path. Gardn does not ship the paired
...
way to force system ConPTY, or Gardn replaces its Windows PTY backend.
...
patch: `apps/gardn/vendor/patches/portable-pty/0002-preserve-raw-windows-command-arguments.patch`

vendored base: `portable-pty 0.9.0`

local file:

- `apps/gardn/vendor/portable-pty/src/cmdbuilder.rs`

reason: Gardn launches custom commands through `cmd.exe /d /c`. Quoting the
...
API or Gardn replaces its Windows PTY backend.
