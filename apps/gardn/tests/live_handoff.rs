...
use support::{
    ...
    register_spawned_gardn_pid, send_input, unregister_spawned_gardn_pid, wait_for_disconnect,
    wait_for_socket,
};
...
fn spawn_server_with_env(
    ...
    cmd.env(
        ...
        runtime_dir.join("gardn-client.sock"),
    );
...
fn spawn_named_session_server(
    ...
    if let Some((api_socket, client_socket)) = inherited_socket_overrides {
        cmd.env("GARDN_SOCKET_PATH", api_socket);
        cmd.env("GARDN_CLIENT_SOCKET_PATH", client_socket);
    } else {
...
fn spawn_default_session_server(config_home: &Path, runtime_dir: &Path) -> SpawnedGardn {
    fs::create_dir_all(config_home.join("gardn-dev")).unwrap();
    fs::create_dir_all(runtime_dir).unwrap();
    fs::write(
        config_home.join("gardn-dev/config.toml"),
...
fn wait_for_replacement_server_pid(runtime_dir: &Path, old_pid: u32, timeout: Duration) -> u32 {
    ...
    while Instant::now() < deadline {
        last_pids = support::gardn_server_pids_for_runtime_dir(runtime_dir).unwrap_or_default();
    ...
    while Instant::now() < deadline {
            ...
            .output()
        {
            ...
            for line in last_stdout.lines() {
                let Some(pid_text) = line.strip_prefix('p') else {
                    continue;
                };
...
fn live_handoff_preserves_explicit_named_session_sockets_despite_inherited_overrides() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let session_dir = config_home.join("gardn-dev/sessions/work");
    let api_socket = session_dir.join("gardn.sock");
    let client_socket = session_dir.join("gardn-client.sock");
    ...
    assert!(
        !config_home.join("gardn-dev/gardn.sock").exists(),
...
fn live_handoff_preserves_installed_plugins() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let api_socket = config_home.join("gardn-dev/gardn.sock");
    let registry_path = config_home.join("gardn-dev/plugins.json");
...
fn named_sessions_share_global_plugin_registry_and_restart_state() {
    ...
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let alpha_socket = config_home.join("gardn-dev/sessions/alpha/gardn.sock");
    let beta_socket = config_home.join("gardn-dev/sessions/beta/gardn.sock");
...
fn live_handoff_accepts_pane_id_from_child_env() {
    ...
    assert_ok(request(
        ...
        serde_json::json!({
            ...
            "method": "pane.send_input",
            "params": {"pane_id": pane_id, "text": format!("printf '%s' \"$GARDN_PANE_ID\" > {}", pane_id_marker.display()), "keys": ["Enter"]}
        }),
    ));
...
fn live_handoff_preserves_http_servers_across_multiple_sessions() {
    ...
    let runtime_dir = base.join("runtime");
    let sessions = [
        (None, config_home.join("gardn-dev/gardn.sock")),
        (
            Some("work"),
            config_home.join("gardn-dev/sessions/work/gardn.sock"),
        ),
    ];
...
fn live_handoff_import_failure_rolls_back_old_server_at(failure_point: &str) {
    ...
    let spawned = spawn_server_with_env(
        ...
        &runtime_dir,
        &api_socket,
        &[("GARDN_TEST_HANDOFF_IMPORT_FAIL", failure_point)],
    );
    wait_for_socket(&api_socket, Duration::from_secs(10));
    register_runtime_dir(&runtime_dir);
