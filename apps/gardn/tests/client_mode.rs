...
use support::{
    ...
    frame_message, read_server_message, register_runtime_dir, register_spawned_gardn_pid,
    unregister_spawned_gardn_pid, wait_for_file, wait_for_socket,
};
...
fn server_unreachable_shows_clear_error() {
    ...
    assert!(
        stderr.contains("Is the Gardn server running? Start it with `gardn server`."),
...
fn attach_thin_client(
...
) -> (SpawnedGardn, SpawnedGardn, SharedOutput) {
    ...
    while Instant::now() < deadline {
        ...
        if out.to_lowercase().contains("gardn:") {
            break;
        }
...
fn assert_client_restores_terminal(trigger: impl FnOnce(&mut SpawnedGardn, &mut SpawnedGardn)) {
    let _lock = test_lock();
    let base = unique_test_dir();
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    ...
    // SpawnedGardn::Drop kills and reaps both processes with a bounded wait.
    drop(spawned_server);
    cleanup_spawned_gardn(thin_client, base);
}
...
fn read_until_client_attaches(client: &SpawnedGardn) -> String {
