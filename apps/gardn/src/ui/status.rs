...
pub(crate) fn toast_notification_rect(
    ...
    let x = match position {
        ToastGardnPosition::TopLeft | ToastGardnPosition::BottomLeft => area.x,
        ToastGardnPosition::TopRight | ToastGardnPosition::BottomRight => {
            area.x + area.width.saturating_sub(width)
        }
    };
    let y = match position {
        ToastGardnPosition::TopLeft | ToastGardnPosition::TopRight => area.y,
