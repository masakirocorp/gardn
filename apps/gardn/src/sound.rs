...
fn temp_sound_path() -> PathBuf {
    ...
    std::env::temp_dir().join(format!("gardn-sound-{}-{id}.mp3", std::process::id()))
}
...
$Path = [Environment]::GetEnvironmentVariable('GARDN_SOUND_PATH', 'Process')
if ([string]::IsNullOrWhiteSpace($Path)) { throw 'GARDN_SOUND_PATH is not set' }
...
#[cfg(any(windows, test))]
fn windows_player_command(path: &Path) -> std::process::Command {
...
mod tests {
    ...
    fn linux_audio_player_does_not_wait_forever() {
        ...
        let status = crate::noninteractive_process::command("kill")
            .args(["-0", pid.trim()])
    ...
    fn linux_audio_player_preserves_completed_output() {
        let player = AudioPlayer {
            ...
            args: &[
                ...
                "gardn-sound-output-test",
            ],
        };

        let output = player
            .output_with_timeout(Path::new("unused.mp3"), std::time::Duration::from_secs(5))
    ...
    fn windows_media_player_uses_process_environment_and_dispatcher() {
        ...
        assert!(script.contains("GetEnvironmentVariable('GARDN_SOUND_PATH', 'Process')"));
