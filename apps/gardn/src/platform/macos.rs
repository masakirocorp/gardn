...
mod tests {
    ...
    #[test]
    fn procargs2_agent_hint_reads_gardn_agent_after_argv() {
        let buf = build_procargs2(
            "/opt/homebrew/bin/nono",
            &["nono", "run", "GARDN_AGENT=codex", "--", "claude"],
            &["PATH=/usr/bin", "GARDN_AGENT=claude", "TERM=xterm-256color"],
        );
    ...
    fn hinted_process_fixture() {
        if std::env::var_os("GARDN_AGENT_HINT_TEST_CHILD").is_some() {
    ...
    fn process_agent_hint_reads_spawned_process_environment() {
        let mut child = Command::new(std::env::current_exe().expect("current test executable"))
            ...
            .arg("--exact")
            .env("GARDN_AGENT_HINT_TEST_CHILD", "1")
            .env("GARDN_AGENT", "claude")
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
