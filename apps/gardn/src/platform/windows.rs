...
const PANE_RUNTIME_MARKER_ENV_VAR: &str = "GARDN_PANE_RUNTIME_ID";
...
pub(crate) fn create_remote_ssh_config_dir(_control_socket_name: &str) -> std::io::Result<PathBuf> {
    ...
    Err(std::io::Error::new(
        ...
        "failed to create private Gardn ssh config directory",
    ))
}
...
fn available_pane_shell_from_snapshot(
    ...
    let name = shell.name.to_ascii_lowercase();
...
pub fn show_desktop_notification(title: &str, body: Option<&str>) -> std::io::Result<bool> {
    ...
    std::thread::Builder::new()
        .name("gardn-windows-notification".into())
...
fn show_desktop_notification_on_thread(
    ...
    let window_name = wide_null("Gardn notifications");
    let hwnd = unsafe {
        CreateWindowExW(
            0,
            class_name.as_ptr(),
    ...
    if unsafe { Shell_NotifyIconW(NIM_MODIFY, &notification) } == 0 {
        ...
        let _ = ready_tx.send(Err(std::io::Error::other(
            "failed to show Gardn desktop notification",
        )));
        return;
    }
...
//
// Windows IMEs live in the terminal-emulator process, not in Gardn. Empirically:
...
/// `SendMessageW` would block Gardn's client thread until that process responds
...
mod tests {
    ...
    const CONSOLE_TEST_CHILD_ENV: &str = "GARDN_TEST_CONSOLE_CHILD_MODE";
    const CONSOLE_TEST_PARENT_PID_ENV: &str = "GARDN_TEST_CONSOLE_PARENT_PID";
    const WMI_DAEMON_TEST_CHILD_ENV: &str = "GARDN_TEST_WMI_DAEMON_CHILD";
    ...
    fn windows_process_cwd_reads_child_launch_directory() {
        let cwd = std::env::temp_dir().join(format!("gardn-cwd-test-{}", std::process::id()));
    ...
    fn process_environment_variable_parser_reads_case_insensitive_marker() {
        let environment: Vec<u16> = "PATH=C:\\Windows\0gardn_pane_runtime_id=pane-a\0\0"
            .encode_utf16()
            .collect();
    ...
    fn private_remote_directory_supports_long_paths() {
        ...
        std::fs::write(private.join("probe"), b"ok").expect("write inherited private file");
