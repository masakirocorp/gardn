...
use crate::api::schema::{
    ErrorResponse, EventsSubscribeParams, Method, Request, SubscriptionEventEnvelope,
    SuccessResponse,
};
...
/// Reusable client for Gardn's newline-delimited JSON API.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    fn local_session_target_resolves_named_session_socket() {
        ...
        assert!(client.socket_path().ends_with("sessions/work/gardn.sock"));
    }

    #[test]
    fn socket_path_target_uses_explicit_path() {
        let path = PathBuf::from("/tmp/gardn-test.sock");
