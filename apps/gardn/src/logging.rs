...
pub(crate) fn help_log_paths_summary() -> String {
    ...
    format!(
        "{} (plus gardn-client.log, gardn-server.log)",
        dir.join("gardn.log").display()
    )
}
...
mod tests {
    ...
    fn temp_log_path(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(unique).join("gardn.log")
    }
    ...
    fn write_replaces_log_without_retained_files_when_size_limit_is_reached() {
        ...
        let writer = RotatingFileMakeWriter::new(dir.clone(), "gardn.log", 8, 0).unwrap();
        {
