...
pub(crate) fn command(
    ...
    let launch = if passthrough_terminal {
        ...
        format!(
            r#"theme_dir="$(mktemp -d "${{TMPDIR:-/tmp}}/gardn-hunk.XXXXXX")" || exec hunk diff --watch --theme auto
config_lock=""
...
cat >> "$theme_dir/hunk/config.toml" <<'GARDN_HUNK_THEME'
{config}
GARDN_HUNK_THEME
...
if [ -n "$user_config_home" ] && [ -f "$theme_dir/hunk/config.toml" ]; then
  ...
  config_lock="$user_config_home/hunk/.gardn-theme-config.lock"
  ...
  if [ "$lock_acquired" -eq 1 ]; then
    ...
    if [ "$can_restore" -eq 1 ] &&
      ...
      if [ -n "$config_destination" ]; then
        config_temp="$(dirname "$config_destination")/.gardn-config.$$"
        if [ "$had_user_config" -eq 1 ]; then
...
mod tests {
    ...
    fn named_theme_wrapper_preserves_user_options_and_persistent_state() {
        let nonce = SystemTime::now()
            ...
            .as_nanos();
        let root =
            std::env::temp_dir().join(format!("gardn-hunk-theme-{}-{nonce}", std::process::id()));
        ...
        let stale_lock = config_dir.join(".gardn-theme-config.lock");
