    ...
    impl PtyIoActor {
        ...
        fn spawn_inner(
            ...
            std::thread::Builder::new()
                .name(format!("gardn-pty-{}", config.pane_id))
        ...
        fn handle_control_command(&mut self, command: PtyIoControlCommand) -> bool {
            match command {
                ...
                PtyIoControlCommand::ForegroundProcessGroup(reply) => {
                    let result = crate::platform::foreground_process_group_id_for_tty_fd(
                        self.file.as_raw_fd(),
                    );
                    let _ = reply.send(result);
                }
        ...
        fn actor_reads_output_while_input_is_backpressured() {
            ...
            let handle = PtyIoActor::spawn(PtyIoActorConfig {
                ...
                on_reader_exit: Some(Box::new(move || {
                    exit_tx.send(()).expect("shutdown receiver alive");
                })),
            })
            .expect("actor spawn");
            ...
            exit_rx
                ...
                .expect("shutdown finishes without deadlock");
        }
