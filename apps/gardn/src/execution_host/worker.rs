...
pub(crate) fn run_from_args(args: &[String]) -> io::Result<()> {
    ...
    if args.iter().any(|arg| arg == "--help" || arg == "-h") {
        eprintln!(
            "usage: gardn execution-worker [--build-info]\n       gardn execution-worker [--protocol-version]\n       gardn execution-worker [--daemon-lifecycle-version]\n       gardn execution-worker --inventory --installation <id> --execution-host <id>\n       gardn execution-worker --retire --installation <id> --execution-host <id>\n       gardn execution-worker --daemon <binding arguments>"
        );
        return Ok(());
    }
