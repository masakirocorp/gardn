...
pub fn config_diagnostic_summary(diagnostics: &[String]) -> Option<String> {
    ...
    Some(format!("config.toml{suffix}; gardn config check"))
}
...
mod tests {
    use super::*;
    #[test]
    fn development_build_uses_gardn_dev_namespace() {
        assert_eq!(app_dir_name(), "gardn-dev");
    }
    ...
    fn config_diagnostic_summary_reports_unknown_keys_compactly() {
        ...
        assert_eq!(
            ...
            Some("config.toml has unknown keys; gardn config check")
        );
    }
    ...
    fn config_diagnostic_summary_keeps_mixed_diagnostics_generic() {
        ...
        assert_eq!(
            config_diagnostic_summary(&diagnostics).as_deref(),
            Some("config.toml; gardn config check")
        );
    }
