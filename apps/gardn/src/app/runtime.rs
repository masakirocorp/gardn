...
mod tests {
    ...
    fn git_refresh_deduplicates_workspaces_with_same_cache_key() {
        let repo =
            std::env::temp_dir().join(format!("gardn-git-refresh-dedupe-{}", std::process::id()));
    ...
    fn git_refresh_items_use_cwd_cache_key_for_non_git_cwd() {
        ...
        let cwd = std::env::temp_dir().join(format!("gardn-non-git-cwd-{}", std::process::id()));
