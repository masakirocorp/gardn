...
//!
//! Gardn is a terminal emulator, so `OSC 0`/`OSC 2` written by a pane
//! stops at Gardn and never reaches the terminal Gardn itself runs
...
impl App {
    ...
    /// Whether `ui.window_title` asks Gardn to own the outer terminal
    /// title at all. When it does not, Gardn leaves whatever the shell
    /// or `ssh` set.
