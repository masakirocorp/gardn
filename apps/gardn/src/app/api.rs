...
impl App {
    ...
    fn handle_internal_event_with_notification_context(
        ...
        if let AppEvent::PaneDied { pane_id, .. } = &ev {
            ...
            if let Some(update) = self.state.publish_pane_process_exit_if_agent(*pane_id) {
                ...
                self.refresh_new_gardn_toast_context_for_update(&update, &previous_toast);
    ...
    pub(crate) fn refresh_new_gardn_toast_context_for_update(
        &mut self,
...
mod tests {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn gardn_toast_context_uses_live_root_runtime_cwd_label() {
        let (_api_tx, api_rx) = tokio::sync::mpsc::unbounded_channel();
        let mut app = App::new(
            &crate::config::Config::default(),
            true,
        ...
        let stale_cwd = temp_root.join("__gardn_original__");
        let live_cwd = temp_root.join("__gardn_projects__");
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn delayed_gardn_toast_context_uses_live_root_runtime_cwd_label() {
        let (_api_tx, api_rx) = tokio::sync::mpsc::unbounded_channel();
        let mut app = App::new(
            &crate::config::Config::default(),
            true,
    ...
    fn terminal_delivery_does_not_refresh_existing_targeted_toast() {
        ...
        app.state.toast = Some(crate::app::state::ToastNotification {
            ...
            context: "__gardn_original__ · 1".into(),
            position: None,
