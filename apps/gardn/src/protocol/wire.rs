//! Wire protocol for Gardn server/client communication.
//!
...
impl ClientInputEvent {
    ...
    pub fn to_raw_input_event(&self) -> crate::raw_input::RawInputEvent {
        match self {
            ...
            Self::TextCommit(text) => crate::raw_input::RawInputEvent::TextCommit(
                crate::input::TextCommit::new(text.clone()),
            ),
            Self::Mouse {
                kind,
                column,
...
pub enum ClientMessage {
    ...
    /// Result of the one armed Gardn-owned direct Kitty transmission.
...
pub enum ServerMessage {
    ...
    WindowTitle {
        /// Sanitized title to write with OSC 0. `None` restores Gardn's default title.
        title: Option<String>,
    },
    ...
    MouseCapture {
        /// True when Gardn mouse UI is enabled or the focused pane app requests mouse reporting.
        enabled: bool,
    ...
    /// One validated Gardn-owned Kitty regular-file RGBA transmission.
    GraphicsFile {
        path: String,
        expected_len: u64,
        image_id: u32,
...
pub fn check_client_version(client_version: u32) -> VersionCheck {
    ...
    if client_version == PROTOCOL_VERSION {
        ...
        VersionCheck::Incompatible(format!(
            "client version {client_version} is older than server version {PROTOCOL_VERSION}; please upgrade your Gardn client"
        ))
    } else {
        VersionCheck::Incompatible(format!(
            "client version {client_version} is newer than server version {PROTOCOL_VERSION}; please upgrade the Gardn server"
        ))
    }
}
...
mod tests {
    ...
    #[test]
    fn client_message_wire_tags_preserve_current_order() {
    ...
    fn client_input_events_preserve_protocol_12_bytes() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                117, 0, 17, 0, 2, 1, 0, 2, 0, 1, 27, 1, 27, 0, 1, 7, 228, 189, 160, 240, 159, 153,
                130, 2, 0, 0, 3, 4, 0,
            ]
        );
    ...
    fn server_window_title_roundtrip() {
        for title in [Some("gardn api".to_owned()), None] {
    ...
    fn server_mouse_capture_roundtrip() {
        ...
        assert_bincode_bytes(&msg, &[0x09, 0x01, 0x01]);
    }
    ...
    fn server_terminal_bell_roundtrip() {
        ...
        assert_bincode_bytes(&msg, &[0x0d, 0x03]);
        let encoded = bincode::serde::encode_to_vec(&msg, bincode::config::standard()).unwrap();
        let (decoded, _): (ServerMessage, _) =
            bincode::serde::decode_from_slice(&encoded, bincode::config::standard()).unwrap();
        assert_eq!(msg, decoded);
    ...
    fn direct_graphics_messages_roundtrip() {
        ...
        let server = ServerMessage::GraphicsFile {
            path: "/run/user/1000/gardn/source/frame".into(),
            expected_len: 4,
            image_id: 42,
            transfer_id: 7,
    ...
    fn version_older_client_rejected() {
        assert_eq!(
            ...
            VersionCheck::Incompatible(format!(
                "client version {} is older than server version {PROTOCOL_VERSION}; please upgrade your Gardn client",
                PROTOCOL_VERSION - 1
            ))
        );
    }
    ...
    fn version_newer_client_rejected() {
        assert_eq!(
            check_client_version(PROTOCOL_VERSION + 1),
            VersionCheck::Incompatible(format!(
                "client version {} is newer than server version {PROTOCOL_VERSION}; please upgrade the Gardn server",
                PROTOCOL_VERSION + 1
            ))
        );
    }
