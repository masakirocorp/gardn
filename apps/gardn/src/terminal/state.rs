...
impl TerminalState {
    ...
    fn resolved_report_session_ref(
        ...
        if source == "gardn:omp" && agent_label == "omp" {
    ...
    fn reported_omp_subagent_ref_would_replace_current(
        ...
        current_ref.is_some_and(|current_ref| {
            Self::session_ref_available_for_report("gardn:omp", "omp", current_ref)
    ...
    fn is_unsequenced_opencode_selection(
        ...
        (source, agent_label, session_start_source, seq)
            == ("gardn:opencode", "opencode", Some("select"), None)
    }
    ...
    pub fn take_missing_integration_warning_agent(&mut self, now: Instant) -> Option<Agent> {
        ...
        if !Self::agent_supports_gardn_integration(agent)
            || self.has_gardn_integration_evidence_for_agent_at(agent, now)
    ...
    pub fn has_gardn_integration_evidence_for_detected_agent_at(&self, now: Instant) -> bool {
        self.detected_agent
            .is_some_and(|agent| self.has_gardn_integration_evidence_for_agent_at(agent, now))
    }

    fn has_gardn_integration_evidence_for_agent_at(&self, agent: Agent, now: Instant) -> bool {
        self.hook_authority.as_ref().is_some_and(|authority| {
            Self::gardn_report_identity_matches_agent(
                &authority.source,
    ...
    fn agent_supports_gardn_integration(agent: Agent) -> bool {
        matches!(
            agent,
            Agent::Pi
                | Agent::OhMyPi
    ...
    fn gardn_report_identity_matches_agent(
        source: &str,
        agent_label: Option<&String>,
        agent: Agent,
    ) -> bool {
        let label = crate::detect::agent_label(agent);
        source == format!("gardn:{label}")
...
mod tests {
    ...
    fn official_release_clears_process_owned_agent_identity() {
        ...
        let mutation = terminal
            .release_agent_with_mutation("gardn:pi", "pi", None, None)
    ...
    fn clearing_hook_authority_clears_session_ref() {
        ...
        let mutation = terminal
            .clear_hook_authority_with_mutation(Some("gardn:pi"), Some(21))
    ...
    fn stale_release_sequence_is_ignored_for_same_source() {
        ...
        let change = terminal.release_agent("gardn:pi", "pi", Some(19));
