...
//!
//! Stored at `~/.config/gardn/session.json`.
//! Optional pane screen history is stored separately at `session-history.json`.
//! Installed plugins are persisted globally at `~/.config/gardn/plugins.json`.
//! SSH connection profiles are persisted globally at `~/.config/gardn/ssh-profiles.json`.
