...
fn remove_value_event_commands(
    ...
    entries.retain_mut(|entry| {
        ...
        let emptied_owned = command_entries.len() != before;
        removed |= emptied_owned;
        !(command_entries.is_empty() && emptied_owned)
    });
...
fn remove_event_commands(
    ...
    for entry in entries.elements() {
        ...
        if removed_from_entry && command_entries.elements().is_empty() {
            entry.remove();
        }
    }
...
mod tests {
    ...
    fn paths() -> (&'static Path, &'static Path) {
        (
            ...
            Path::new("/home/test/.claude/hooks/gardn-agent-state.sh"),
        )
    }
