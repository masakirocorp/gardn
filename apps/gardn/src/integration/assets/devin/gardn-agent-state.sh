...
action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/gardn-devin-hook.XXXXXX")" || exit 0
...
def load_session_list(project_dir):
    injected = os.environ.get("GARDN_DEVIN_LIST_JSON")
...
hook_input = load_hook_input(os.environ.get("GARDN_HOOK_INPUT_FILE"))
