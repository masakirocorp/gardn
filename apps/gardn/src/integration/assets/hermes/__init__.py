"""Hermes plugin installed by Gardn to report resumable session identity."""

# GARDN_INTEGRATION_ID=hermes
...
_SOURCE = "gardn:hermes"
_AGENT = "hermes"
_INTERACTIVE_PLATFORMS = {"cli", "tui", "desktop", "acp"}


def _base_params() -> tuple[str, str] | None:
    if os.environ.get("GARDN_ENV") != "1":
        return None
    pane_id = os.environ.get("GARDN_PANE_ID", "").strip()
    socket_path = os.environ.get("GARDN_SOCKET_PATH", "").strip()
