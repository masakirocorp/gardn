...
# GARDN_INTEGRATION_ID=qodercli
# GARDN_INTEGRATION_VERSION=1
#
# Reports qodercli agent state changes to Gardn. Registered as a Command hook
# in ~/.qoder/settings.json by `gardn integration install qodercli` and
...
# `assets/claude/gardn-agent-state.sh` already consumes claude code's stdin
# payload. No environment variable is consulted for the event identity.

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/gardn-qodercli-hook.XXXXXX")" || exit 0
