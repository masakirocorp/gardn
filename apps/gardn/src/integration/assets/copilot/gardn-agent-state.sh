...
# GARDN_INTEGRATION_ID=copilot
# GARDN_INTEGRATION_VERSION=1

set -eu

hook_input_file="$(mktemp "${TMPDIR:-/tmp}/gardn-copilot-hook.XXXXXX")" || exit 0
...
GARDN_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
import json
import os
import random
import socket
...
try:
    ...
    if event_key == "sessionstart":
        ...
        # ownership until a real session exit so Gardn can still track later turns.
