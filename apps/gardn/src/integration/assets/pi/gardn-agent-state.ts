...
export default function (pi) {
  ...
  const instanceId = Symbol("gardn-pi-agent");
  let blockedCount = 0;
  ...
  function activateRootSession(ctx: unknown, sessionStartSource = "startup"): boolean {
    // TUI only: RPC/JSON/print modes are headless (no PTY Gardn can display),
