...
use super::snapshot::{
    PaneAgentSessionSnapshot, PaneHistorySnapshot, TabHistorySnapshot, TerminalTabSnapshot,
...
pub fn handoff_pane_aliases(
    ...
    for (ws_idx, ws_snap) in snapshot.workspaces.iter().enumerate() {
        let workspace = ws_snap
            .id
            .as_ref()
            ...
            .or_else(|| {
                ws_snap
                    .id
                    .is_none()
                    .then(|| workspaces.get(ws_idx))
                    .flatten()
            });
        let Some(workspace) = workspace else {
            continue;
        };
        for (idx, tab_snap) in ws_snap.tabs.iter().enumerate() {
            let public_number = ws_snap
                .public_tab_numbers
                .get(idx)
                .copied()
                .unwrap_or(idx + 1);
            let Some(tab) = workspace
                .tabs
                .iter()
                .find(|tab| tab.number() == public_number)
            else {
                continue;
            };
            let (tab_snap, tab) = match (tab_snap, tab) {
                (TabSnapshot::Terminal(tab_snap), WorkspaceTab::Terminal(tab)) => (tab_snap, tab),
                _ => continue,
            };
...
fn migrated_public_pane_numbers_by_old_raw(
    ...
    for tab in &snap.tabs {
        let pane_ids = match tab {
            TabSnapshot::Terminal(tab) => collect_snapshot_pane_ids(&tab.layout),
            TabSnapshot::Github {
                legacy_pane_ids, ..
            } => legacy_pane_ids.clone(),
        };
...
fn restore_workspace(
    ...
    for (idx, tab_snap) in snap.tabs.iter().enumerate() {
        ...
        if let TabSnapshot::Github {
            custom_name,
            legacy_pane_ids,
        } = tab_snap
        {
            for pane_id in legacy_pane_ids {
                if let Some(imported) = imported_panes.remove(pane_id) {
                    imported.close_imported_descriptor();
                }
            }
            let tab = WorkspaceTab::Github(crate::workspace::GithubTab {
                number: tab_number,
        ...
        let TabSnapshot::Terminal(tab_snap) = tab_snap else {
            continue;
        };
...
fn restore_tab(
    ...
    resumed_agent_sessions: &mut HashSet<String>,
    imported_panes: &mut HashMap<u32, ImportedPaneRuntime>,
    public_pane_numbers_by_old_raw: &HashMap<u32, usize>,
    ...
    for id in &pane_ids {
        ...
        let public_pane_number =
            old_pane_id.and_then(|old_id| public_pane_numbers_by_old_raw.get(&old_id).copied());
        let launch_env = public_pane_number
            .map(|pane_number| {
    ...
    (
        Some((
            WorkspaceTab::Terminal(crate::workspace::Tab {
                custom_name: snap.custom_name.clone(),
                number,
                root_pane,
                layout,
...
mod tests {
    ...
    fn handoff_pane_aliases_remap_previous_env_aliases_transitively() {
        ...
        let restored_pane = workspace.terminal_tab(0).unwrap().root_pane;
    ...
    fn restore_falls_back_to_home_when_cwd_missing() {
        let missing =
            std::env::temp_dir().join(format!("gardn-missing-restore-cwd-{}", std::process::id()));
    ...
    #[tokio::test]
    async fn restore_preserves_public_tab_and_native_identity_after_remap() {
        ...
        let pane_numbers: Vec<_> = workspace.public_pane_numbers.values().copied().collect();
        assert!(pane_numbers.is_empty());
        assert_eq!(workspace.next_public_pane_number, 4);
        assert_eq!(workspace.tabs[0].number(), 5);
        assert!(workspace.tabs[0].is_github());
        assert_eq!(workspace.next_public_tab_number, 6);
    }

    #[test]
    fn legacy_restore_assigns_missing_public_pane_numbers_without_reuse() {
        let cwd = std::env::current_dir().unwrap();
    ...
    fn restore_plan_preserves_available_launch_command_for_every_resumable_agent() {
        ...
        let omp_session_path = std::env::temp_dir().join(format!(
            "gardn-restore-existing-omp-{}-{}.jsonl",
            std::process::id(),
    ...
    fn restore_rehydrates_agent_session_metadata() {
        ...
        assert_eq!(preserved.source, "gardn:hermes");
    ...
    async fn restore_carries_persisted_agent_session_metadata() {
        ...
        assert_eq!(session.source, "gardn:opencode");
    ...
    #[tokio::test]
    #[cfg(unix)]
    async fn native_agent_restore_defers_resume_through_shell() {
        let cwd = std::env::current_dir().unwrap();
        let snapshot = SessionSnapshot {
            version: super::super::snapshot::SNAPSHOT_VERSION,
            session_namespace_id: "session-test".to_string(),
        ...
        assert!(
            runtimes.is_empty(),
            "native agent restore should wait for app render before spawning a shell runtime"
        );
        let terminal = terminals
            .values()
            .next()
        ...
        assert!(
            terminal.pending_agent_resume_plan.is_some(),
            "restored native agent panes should defer resume command injection into a shell"
        );
        assert!(!terminal.respawn_shell_on_exit);
    }
    ...
    fn restore_repairs_poisoned_omp_profile_context() {
        ...
        let TabSnapshot::Terminal(tab) = &mut snapshot.workspaces[0].tabs[0] else {
            panic!("snapshot should contain a terminal tab");
        };
        let pane = tab
            .panes
            .values_mut()
