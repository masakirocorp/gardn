...
impl TerminalKey {
    ...
    pub(crate) fn with_shifted_codepoint_opt(mut self, shifted_codepoint: Option<u32>) -> Self {
    ...
    pub(crate) fn identity(&self) -> KeyIdentity {
        match &self.source {
            ...
            | KeySource::Vt { .. } => KeyIdentity::Semantic(self.semantic_identity_code()),
        }
    }
    ...
    fn semantic_identity_code(&self) -> KeyCode {
        match self.code {
            KeyCode::Char(ch) if ch.is_ascii_alphabetic() => KeyCode::Char(ch.to_ascii_lowercase()),
            code => code,
        }
    }
