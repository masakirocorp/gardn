...
pub(crate) fn command(
    ...
    let launch = if passthrough_terminal {
        ...
        format!(
            r#"theme_dir="$(mktemp -d "${{TMPDIR:-/tmp}}/gardn-lazygit.XXXXXX")" || exec lazygit
cleanup() {{
  rm -rf "$theme_dir"
}}
trap cleanup EXIT INT TERM
cat > "$theme_dir/theme.yml" <<'GARDN_LAZYGIT_THEME'
{config}
GARDN_LAZYGIT_THEME
...
mod tests {
    ...
    fn render_missing_screen(script: String) -> String {
        ...
        let output = child.wait_with_output().expect("wait for lazygit wrapper");
    ...
    #[test]
    fn named_theme_launch_merges_user_config_before_oh_my_gardn_overlay() {
        let command = command(
            &Palette::tokyo_night(),
    ...
    #[test]
    fn missing_lazygit_screen_is_rendered_with_install_source() {
