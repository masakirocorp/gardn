...
mod tests {
    ...
    fn probe_bottom_hard_deadline_wins_over_synchronized_output() {
        ...
        runtime.test_process_pty_bytes(crate::layout::PaneId::from_raw(0), b"\x1b[?2026h");
        assert!(pending
            .poll(Some(&runtime), started + MAX_DURATION)
            .is_none());
        assert_eq!(
    ...
    fn redraw_events_advance_harvest_and_restore_without_settle_delays() {
        ...
        let (runtime, mut input_rx) = TerminalRuntime::test_with_channel_capacity(20, 5, 8);
        runtime.test_process_pty_bytes(crate::layout::PaneId::from_raw(0), &initial_bytes);
        let started = Instant::now();
        let (pending, response_rx) = pending_read(&runtime, started, 8);
    ...
    fn unrelated_output_does_not_retry_restore_before_the_step_timeout() {
        ...
        let (runtime, mut input_rx) = TerminalRuntime::test_with_channel_capacity(20, 5, 8);
        runtime.test_process_pty_bytes(crate::layout::PaneId::from_raw(0), &draw(&initial, true));
        let started = Instant::now();
        let (pending, response_rx) = pending_read(&runtime, started, 8);
        let harvest_started = started + INITIAL_QUIET + STEP_TIMEOUT;
        ...
        runtime.test_process_pty_bytes(crate::layout::PaneId::from_raw(0), &draw(&initial, false));
        let pending = pending
    ...
    fn synchronized_redraw_is_not_consumed_after_the_step_timeout() {
        ...
        runtime.test_process_pty_bytes(crate::layout::PaneId::from_raw(0), &synchronized);
        let pending = pending
            .poll(Some(&runtime), harvest_started + Duration::from_millis(1))
        ...
        runtime.test_process_pty_bytes(crate::layout::PaneId::from_raw(0), b"\x1b[?2026l");
        let pending = pending
            .poll(
                Some(&runtime),
    ...
    fn aligned_intermediate_redraw_is_coalesced_before_scrolling_again() {
        ...
        runtime.test_process_pty_bytes(
            crate::layout::PaneId::from_raw(0),
            &draw(&["13", "14", "15", "16", "17", "loading"], false),
        );
        let pending = pending
            .poll(Some(&runtime), harvest_started + Duration::from_millis(1))
        ...
        runtime.test_process_pty_bytes(
            crate::layout::PaneId::from_raw(0),
            &draw(&["13", "14", "15", "16", "17", "ready"], false),
        );
        let pending = pending
    ...
    fn incomplete_redraw_waits_for_an_aligned_screen_without_resetting_timeout() {
        ...
        runtime.test_process_pty_bytes(crate::layout::PaneId::from_raw(0), &draw(&["13"], false));
        let pending = pending
            .poll(
                Some(&runtime),
                started + INITIAL_QUIET + STEP_TIMEOUT + Duration::from_millis(1),
