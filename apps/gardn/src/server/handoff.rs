...
pub(crate) struct HandoffManifest {
    ...
    /// An outer window title set over the API outlives the server that took
    /// the call, so a handoff carries it rather than falling back to the
    /// config. Absent from manifests written before this field existed.
    #[serde(default)]
...
pub(crate) fn handoff_socket_path() -> PathBuf {
    crate::session::data_dir().join(format!("gardn-handoff-{}.sock", std::process::id()))
}
...
pub(crate) fn receive(socket_path: &Path, token: &str) -> io::Result<ReceivedHandoff> {
    ...
    {
        return Err(io::Error::other(format!(
            "handoff expected gardn v{}, but this server is v{}",
...
mod window_title_tests {
    ...
    fn empty_snapshot() -> crate::persist::SessionSnapshot {
        serde_json::from_value(serde_json::json!({ "workspaces": [] }))
