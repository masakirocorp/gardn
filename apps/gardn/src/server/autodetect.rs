//! Auto-detect launch behavior for the `gardn` command.
//!
//! When the user runs `gardn` with no subcommand:
...
/// Spawns the Gardn server as a background daemon process.
///
...
/// - Inherits relevant environment variables (`XDG_CONFIG_HOME`, `GARDN_SESSION`,
...
pub fn spawn_server_daemon() -> io::Result<u32> {
    let exe = std::env::current_exe().map_err(|err| {
        io::Error::new(
            err.kind(),
            format!("failed to determine Gardn executable path: {err}"),
        )
    })?;
...
pub fn wait_for_server_socket(socket_path: &Path, timeout: Duration) -> io::Result<()> {
    ...
    Err(io::Error::new(
        ...
        format!(
            "server did not become ready within {}s (socket: {}). The background server may still be starting; try `gardn` again, or check {}",
            timeout.as_secs(),
            socket_path.display(),
...
mod tests {
    ...
    fn server_daemon_command_clears_socket_overrides_for_explicit_session() {
        ...
        let command = build_server_daemon_command(PathBuf::from("/tmp/gardn-test"));
        ...
        assert!(envs.iter().any(|(key, value)| {
            *key == OsStr::new("GARDN_CLIENT_SOCKET_PATH") && value.is_none()
        }));
        crate::session::clear_explicit_session_for_test();
    }
    ...
    fn wait_for_server_socket_times_out() {
        ...
        assert!(err.to_string().contains("gardn-server.log"));
        let _ = std::fs::remove_dir_all(dir);
    }

    #[test]
    fn default_server_startup_wait_allows_slow_local_launches() {
