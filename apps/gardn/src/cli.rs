...
fn run_status_command(args: &[String]) -> std::io::Result<i32> {
    match args.first().map(|arg| arg.as_str()) {
        ...
        Some("server") => {
            let Ok(format) = parse_status_format(&args[1..], "usage: gardn status server [--json]")
            else {
                return Ok(2);
            };
        ...
        Some("client") => {
            let Ok(format) = parse_status_format(&args[1..], "usage: gardn status client [--json]")
            else {
                return Ok(2);
            };
...
fn config_check(args: &[String]) -> std::io::Result<i32> {
    match args {
        ...
        [flag] if matches!(flag.as_str(), "help" | "--help" | "-h") => {
            eprintln!("usage: gardn config check");
            return Ok(0);
        }
        _ => {
            eprintln!("usage: gardn config check");
...
fn print_notification_help() {
    eprintln!(
        "usage: gardn notification show <title> [--body TEXT] [--position top-left|top-right|bottom-left|bottom-right] [--sound none|done|request]"
    );
}
    ...
    while idx < args.len() {
        match args[idx].as_str() {
            ...
            "--position" => {
                ...
                position = Some(parse_gardn_toast_position(value)?);
                idx += 2;
            }
            "--sound" => {
                let Some(value) = args.get(idx + 1) else {
...
fn parse_gardn_toast_position(value: &str) -> std::io::Result<crate::api::schema::ToastPosition> {
    match value {
...
fn agent_explain(args: &[String]) -> std::io::Result<i32> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!("usage: gardn agent explain <target> [--json]");
                eprintln!("usage: gardn agent explain --file PATH --agent LABEL [--json]");
                return Ok(0);
            }
    ...
    let explain = if let Some(path) = file {
        ...
        let Some(agent_label) = agent else {
            eprintln!("gardn agent explain --file requires --agent LABEL");
            return Ok(2);
        };
        let content = std::fs::read_to_string(path)?;
...
fn server_stop(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: gardn server stop");
        return Ok(2);
    }
...
fn server_reload_config(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: gardn server reload-config");
        return Ok(2);
    }
...
fn server_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: gardn server agent-manifests");
        return Ok(2);
    }
...
fn server_reload_agent_manifests(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: gardn server reload-agent-manifests");
        return Ok(2);
    }
...
fn server_live_handoff(args: &[String]) -> std::io::Result<i32> {
    let Some(params) = parse_live_handoff_params(args) else {
        eprintln!(
            "usage: gardn server live-handoff [--import-exe <path>] [--expected-protocol <n>] [--expected-version <version>]"
        );
        return Ok(2);
    };
...
fn session_attach_help(args: &[String]) -> std::io::Result<i32> {
    if matches!(
        ...
        eprintln!("usage: gardn session attach <name>");
        return Ok(0);
    }
    eprintln!("usage: gardn session attach <name>");
    Ok(2)
...
fn session_list(args: &[String]) -> std::io::Result<i32> {
    let json = match parse_session_json_only(args, "usage: gardn session list [--json]") {
        Ok(json) => json,
...
fn session_stop(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: gardn session stop <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn session_delete(args: &[String]) -> std::io::Result<i32> {
    let (name, json) =
        match parse_session_name_and_json(args, "usage: gardn session delete <name> [--json]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn group_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: gardn group list");
        return Ok(2);
    }
...
fn group_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_group_id) = args.first() else {
        eprintln!("usage: gardn group focus <group_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn group focus <group_id>");
...
fn group_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: gardn group rename <group_id> <name>");
        return Ok(2);
    }
...
fn group_delete(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_group_id) = args.first() else {
        eprintln!("usage: gardn group delete <group_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn group delete <group_id>");
...
fn agent_start(args: &[String]) -> std::io::Result<i32> {
    ...
    let Some(name) = args.first() else {
        eprintln!("usage: gardn agent start <name> [--cwd PATH] [--host EXECUTION_HOST_ID] [--workspace ID] [--tab ID] [--split right|down] [--focus|--no-focus] -- <argv...>");
        return Ok(2);
    };
...
fn agent_list(args: &[String]) -> std::io::Result<i32> {
    if let Some(code) = agent_subcommand_help(args, "usage: gardn agent list") {
        return Ok(code);
    }

    if !args.is_empty() {
        eprintln!("usage: gardn agent list");
        return Ok(2);
    }
...
fn agent_get(args: &[String]) -> std::io::Result<i32> {
    if let Some(code) = agent_subcommand_help(args, "usage: gardn agent get <target>") {
        return Ok(code);
    }

    let Some(target) = args.first() else {
        eprintln!("usage: gardn agent get <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn agent get <target>");
...
fn agent_focus(args: &[String]) -> std::io::Result<i32> {
    if let Some(code) = agent_subcommand_help(args, "usage: gardn agent focus <target>") {
        return Ok(code);
    }

    let Some(target) = args.first() else {
        eprintln!("usage: gardn agent focus <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn agent focus <target>");
...
fn agent_attach(args: &[String]) -> std::io::Result<i32> {
    ...
    let (target, takeover) =
        match parse_attach_target(args, "usage: gardn agent attach <target> [--takeover]") {
            Ok(parsed) => parsed,
            Err(code) => return Ok(code),
        };
...
fn agent_wait(args: &[String]) -> std::io::Result<i32> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!("usage: gardn agent wait <target> --status <idle|working|blocked|unknown> [--timeout MS]");
                return Ok(0);
            }
            other => {
                eprintln!("unknown option: {other}");
...
fn terminal_attach(args: &[String]) -> std::io::Result<i32> {
    let (terminal_id, takeover) = match parse_attach_target(
        args,
        "usage: gardn terminal attach <terminal_id> [--takeover]",
    ) {
        Ok(parsed) => parsed,
        Err(code) => return Ok(code),
    };
...
fn terminal_title(args: &[String]) -> std::io::Result<i32> {
    match args.first().map(|arg| arg.as_str()) {
        Some("set") => {
            if args.len() != 2 {
                eprintln!("usage: gardn terminal title set <title>");
                return Ok(2);
            }
            print_response(&send_request(&Request {
        ...
        Some("clear") => {
            if args.len() != 1 {
                eprintln!("usage: gardn terminal title clear");
                return Ok(2);
            }
            print_response(&send_request(&Request {
        ...
        Some("help" | "--help" | "-h") => {
            eprintln!("usage: gardn terminal title set <title>");
            eprintln!("       gardn terminal title clear");
            Ok(0)
        }
        _ => {
            eprintln!("usage: gardn terminal title set <title>");
...
fn agent_rename(args: &[String]) -> std::io::Result<i32> {
    if let Some(code) =
        agent_subcommand_help(args, "usage: gardn agent rename <target> <name>|--clear")
    {
        return Ok(code);
    }

    let Some(target) = args.first() else {
        eprintln!("usage: gardn agent rename <target> <name>|--clear");
        return Ok(2);
    };
    if args.len() < 2 {
        eprintln!("usage: gardn agent rename <target> <name>|--clear");
...
fn agent_prompt(args: &[String]) -> std::io::Result<i32> {
    ...
    let Some(target) = args.first() else {
        eprintln!("usage: gardn agent prompt <target> <text> [--wait-for STATUS] [--timeout MS]");
        return Ok(2);
    };
...
fn agent_send_keys(args: &[String]) -> std::io::Result<i32> {
    ...
    let Some(target) = args.first() else {
        eprintln!("usage: gardn agent send-keys <target> <key>...");
        return Ok(2);
    };
    if args.len() < 2 {
        eprintln!("usage: gardn agent send-keys <target> <key>...");
...
fn agent_read(args: &[String]) -> std::io::Result<i32> {
    ...
    let Some(target) = args.first() else {
        eprintln!("usage: gardn agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
        return Ok(2);
    };
...
fn parse_integration_target(
    ...
    let Some(target) = args.first().map(|arg| arg.as_str()) else {
        eprintln!(
            "usage: gardn integration {action} <pi|omp|claude|codex|devin|opencode|hermes|grok>"
        );
        return Ok(None);
    };
    if args.len() != 1 {
...
fn parse_wait_output_args(args: &[String]) -> Result<PaneWaitForOutputParams, String> {
    const USAGE: &str = "usage: gardn wait output <pane_id> --match <text> [--source visible|recent|recent-unwrapped] [--lines N] [--timeout MS] [--regex] [--raw]";
...
fn wait_agent_status(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: gardn wait agent-status <pane_id> --status <idle|working|blocked|done|unknown> [--timeout MS]");
        return Ok(2);
    };
...
fn print_server_help() {
    eprintln!("gardn server commands:");
    eprintln!("  gardn server                run as headless server");
    eprintln!("  gardn server stop           stop the running server via the API socket");
    eprintln!("  gardn server live-handoff   hand off live panes to a new local server");
    eprintln!("  gardn server reload-config  reload config.toml in the running server");
    eprintln!("  gardn server agent-manifests         list active agent detection manifests");
    eprintln!("  gardn server reload-agent-manifests  reload local agent detection manifests");
}
...
fn print_config_help() {
    eprintln!("gardn config commands:");
    eprintln!("  gardn config reset-keys  back up config.toml and remove custom keybindings");
    eprintln!("  gardn config check  validate config.toml and print diagnostics");
}

fn print_group_help() {
    eprintln!("gardn group commands:");
    eprintln!("  gardn group list");
    eprintln!("  gardn group create <name> [--cwd PATH] [--host EXECUTION_HOST_ID]");
    eprintln!("  gardn group focus <group_id>");
    eprintln!("  gardn group switch <group_id>");
    eprintln!("  gardn group rename <group_id> <name>");
    eprintln!("  gardn group delete <group_id>");
}

fn print_agent_help() {
    eprintln!("gardn agent commands:");
    eprintln!("  gardn agent list");
    eprintln!("  gardn agent get <target>");
    eprintln!("  gardn agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!("  gardn agent prompt <target> <text> [--wait-for STATUS] [--timeout MS]");
    eprintln!("  gardn agent send-keys <target> <key>...");
    eprintln!("  gardn agent rename <target> <name>|--clear");
    eprintln!("  gardn agent focus <target>");
    eprintln!("  gardn agent wait <target> --status <idle|working|blocked|unknown> [--timeout MS]");
    eprintln!("  gardn agent attach <target> [--takeover]");
    eprintln!("  gardn agent start <name> [--cwd PATH] [--host EXECUTION_HOST_ID] [--workspace ID] [--tab ID] [--split right|down] [--focus|--no-focus] -- <argv...>");
    eprintln!("  gardn agent explain <target> [--json]");
    eprintln!("  gardn agent explain --file PATH --agent LABEL [--json]");
...
fn print_terminal_help() {
    eprintln!("gardn terminal commands:");
    eprintln!("  gardn terminal attach <terminal_id> [--takeover]");
    eprintln!("  gardn terminal title set <title>");
    eprintln!("  gardn terminal title clear");
    eprintln!("  detach from direct attach with ctrl+b q; send literal ctrl+b with ctrl+b ctrl+b");
}

fn print_wait_help() {
    eprintln!("gardn wait commands:");
    eprintln!("  gardn wait output <pane_id> --match <text> [--source visible|recent|recent-unwrapped] [--lines N] [--timeout MS] [--regex] [--raw]");
    eprintln!(
        "  gardn wait agent-status <pane_id> --status <idle|working|blocked|done|unknown> [--timeout MS]"
    );
}
...
fn print_session_help() {
    eprintln!("gardn session commands:");
    eprintln!("  gardn session list [--json]");
    eprintln!("  gardn session attach <name>");
    eprintln!("  gardn session stop <name> [--json]");
    eprintln!("  gardn session delete <name> [--json]");
...
mod tests {
    ...
    fn parse_env_assignment_accepts_empty_values() {
        assert_eq!(
            super::parse_env_assignment("GARDN_ROLE=").unwrap(),
            ("GARDN_ROLE".to_string(), String::new())
        );
    }
    ...
    fn parse_env_assignment_requires_key_value_separator() {
        assert_eq!(
            super::parse_env_assignment("GARDN_ROLE").unwrap_err(),
