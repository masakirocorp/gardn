...
impl Terminal {
        ...
        F: FnMut(&[u8]) + Send + 'static,
    {
        ...
        let userdata = (&mut *self.callback_state as *mut TerminalCallbackState).cast::<c_void>();
        unsafe {
            ffi::ghostty_terminal_set(
                self.raw,
                ffi::GhosttyTerminalOption_GHOSTTY_TERMINAL_OPT_USERDATA,
...
fn grid_ref_wide(grid_ref: &ffi::GhosttyGridRef) -> Result<CellWide, Error> {
    let cell = grid_ref_cell(grid_ref)?;
...
fn grid_ref_cell(grid_ref: &ffi::GhosttyGridRef) -> Result<ffi::GhosttyCell, Error> {
    let mut cell = 0;
    unsafe {
        ffi::ghostty_grid_ref_cell(grid_ref, &mut cell).into_result()?;
    }
    Ok(cell)
}
...
fn kitty_virtual_cell(
    ...
    grapheme_text: &mut String,
    graphemes: &mut Vec<u32>,
) -> Result<Option<KittyVirtualCell>, Error> {
    cells.grapheme_text_into(grapheme_bytes, grapheme_text)?;
    graphemes.clear();
...
fn kitty_placeholder_diacritic_index(codepoint: u32) -> Option<u32> {
    let map = KITTY_PLACEHOLDER_DIACRITICS.get_or_init(|| {
        // Reuse Ghostty's vendored table so Gardn decodes the same placeholder
...
mod tests {
    ...
    fn kitty_graphics_file_medium_rgba_placement_is_queryable() {
        use base64::Engine;
        let dir =
            std::env::temp_dir().join(format!("gardn-kitty-file-medium-{}", std::process::id()));
