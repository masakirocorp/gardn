...
fn plugin_link(args: &[String]) -> std::io::Result<i32> {
    let Some(path) = args.first() else {
        eprintln!("usage: gardn plugin link <path> [--disabled]");
        return Ok(2);
    };
...
fn plugin_unlink(args: &[String]) -> std::io::Result<i32> {
    let Some(plugin_id) = args.first() else {
        eprintln!("usage: gardn plugin unlink <plugin_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn plugin unlink <plugin_id>");
...
fn plugin_install(args: &[String]) -> std::io::Result<i32> {
    let Some(source_arg) = args.first() else {
        eprintln!("usage: gardn plugin install <owner>/<repo>[/subdir...] [--ref REF] [--yes]");
        return Ok(2);
    };
...
fn plugin_uninstall(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: gardn plugin uninstall <plugin_id|owner/repo[/subdir...]>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn plugin uninstall <plugin_id|owner/repo[/subdir...]>");
...
fn plugin_action_invoke(args: &[String]) -> std::io::Result<i32> {
    let Some(action_id) = args.first() else {
        eprintln!("usage: gardn plugin action invoke <action_id> [--plugin ID]");
        return Ok(2);
    };
    let mut plugin_id = None;
    let mut index = 1;
...
fn plugin_pane_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(pane_id) = args.first() else {
        eprintln!("usage: gardn plugin pane focus <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn plugin pane focus <pane_id>");
...
fn plugin_pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(pane_id) = args.first() else {
        eprintln!("usage: gardn plugin pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn plugin pane close <pane_id>");
...
impl GithubPluginSource {
    fn parse(value: &str) -> Result<Self, String> {
        ...
        if parts.len() < 2 {
            return Err("usage: gardn plugin install <owner>/<repo>[/subdir...]".into());
        }
        let owner = parts[0];
        let repo = parts[1];
...
fn verify_plugin_link_source_response(
        ...
        || plugin.source.managed_path != expected.managed_path
    {
        return Err(std::io::Error::other(
            "running Gardn server did not persist GitHub plugin source metadata",
        ));
    }
    Ok(())
}
...
fn ensure_manifest_unchanged_after_build(
    ...
    Err(io::Error::other(
        "plugin build changed gardn-plugin.toml after install preview; aborting install",
    ))
}
...
fn run_plugin_build_command(
    ...
    child
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    scrub_gardn_runtime_env(&mut child);
...
fn print_plugin_help() {
    eprintln!("gardn plugin commands:");
    eprintln!("  gardn plugin install <owner>/<repo>[/subdir...] [--ref REF] [--yes]");
    eprintln!("  gardn plugin uninstall <plugin_id|owner/repo[/subdir...]>");
    eprintln!("  gardn plugin link <path> [--disabled]");
    eprintln!("  gardn plugin list [--plugin ID] [--json]");
...
fn print_plugin_action_help() {
    eprintln!("gardn plugin action commands:");
    eprintln!("  gardn plugin action list [--plugin ID]");
    eprintln!("  gardn plugin action invoke <action_id> [--plugin ID]");
}
    ...
    fn github_plugin(
        ...
        InstalledPluginInfo {
            ...
            source: PluginSourceInfo {
                ...
                managed_path: Some(format!("/tmp/gardn/plugins/{id}")),
