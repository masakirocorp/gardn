...
fn pane_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: gardn pane get <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn pane get <pane_id>");
...
fn pane_current(args: &[String]) -> std::io::Result<i32> {
    let env_pane_id = std::env::var("GARDN_PANE_ID")
        .ok()
...
fn parse_pane_focus_args(args: &[String]) -> Result<PaneFocusDirectionParams, String> {
    let params = parse_pane_neighbor_args(args).map_err(|_| {
        "usage: gardn pane focus --direction left|right|up|down [--pane ID|--current]".to_string()
    })?;
...
fn parse_pane_resize_args(args: &[String]) -> Result<PaneResizeParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: gardn pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
                .into(),
        );
    };
...
fn pane_rename(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: gardn pane rename <pane_id> <label>|--clear");
        return Ok(2);
    };
    if args.len() < 2 {
        eprintln!("usage: gardn pane rename <pane_id> <label>|--clear");
...
fn parse_pane_read_args(args: &[String]) -> Result<PaneReadParams, String> {
    const USAGE: &str = "usage: gardn pane read <pane_id> [--source visible|recent|recent-unwrapped|detection] [--lines N] [--format text|ansi] [--ansi] [--raw]";
...
fn parse_pane_split_args(args: &[String]) -> Result<PaneSplitParams, String> {
    ...
    let Some(direction) = direction else {
        return Err(
            "usage: gardn pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--host EXECUTION_HOST_ID] [--env KEY=VALUE] [--focus] [--no-focus]"
                .into(),
        );
    };
...
fn pane_move_usage() -> String {
    "usage: gardn pane move <pane_id> --tab <tab_id> --split right|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]\n       gardn pane move <pane_id> --new-tab [--workspace ID] [--label TEXT] [--focus|--no-focus]\n       gardn pane move <pane_id> --new-workspace [--label TEXT] [--tab-label TEXT] [--focus|--no-focus]"
        .into()
}
...
fn parse_pane_swap_args(args: &[String]) -> Result<PaneSwapParams, String> {
    ...
    match (directional, explicit) {
        ...
        _ => Err(
            "usage: gardn pane swap --direction left|right|up|down [--pane ID|--current]\n       gardn pane swap --source-pane ID --target-pane ID"
                .into(),
        ),
    }
}
...
fn pane_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: gardn pane close <pane_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn pane close <pane_id>");
...
fn pane_send_text(args: &[String]) -> std::io::Result<i32> {
    if matches!(
        ...
        eprintln!("usage: gardn pane send-text <pane_id> <text>");
        eprintln!("next: gardn pane run <PANE_ID> <COMMAND> sends text and Enter in one call");
        return Ok(0);
    }
    if args.len() < 2 {
        eprintln!("usage: gardn pane send-text <pane_id> <text>");
...
fn pane_send_keys(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: gardn pane send-keys <pane_id> <key> [key ...]");
        return Ok(2);
    }
...
fn pane_run(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: gardn pane run <pane_id> <command>");
        return Ok(2);
    }
...
fn pane_report_agent_session(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: gardn pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH] [--session-start-source SOURCE]");
        return Ok(2);
    };
...
fn pane_release_agent(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: gardn pane release-agent <pane_id> --source ID --agent LABEL [--seq N]");
        return Ok(2);
    };
...
fn pane_report_metadata(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_pane_id) = args.first() else {
        eprintln!("usage: gardn pane report-metadata <pane_id> --source ID [--agent LABEL] [--applies-to-source ID] [--title TEXT|--clear-title] [--display-agent TEXT|--clear-display-agent] [--custom-status TEXT|--clear-custom-status] [--state-label STATUS=TEXT] [--clear-state-labels] [--seq N] [--ttl-ms N]");
        return Ok(2);
    };
...
fn print_pane_help() {
    eprintln!("gardn pane commands:");
    eprintln!("  gardn pane list [--workspace <workspace_id>]");
    eprintln!("  gardn pane current [--pane ID|--current]");
    eprintln!("  gardn pane get <pane_id>");
    eprintln!("  gardn pane layout [--pane ID|--current]");
    eprintln!("  gardn pane process-info [--pane ID|--current]");
    eprintln!("  gardn pane neighbor --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  gardn pane edges [--pane ID|--current]");
    eprintln!("  gardn pane focus --direction left|right|up|down [--pane ID|--current]");
    eprintln!(
        "  gardn pane resize --direction left|right|up|down [--amount FLOAT] [--pane ID|--current]"
    );
    eprintln!("  gardn pane zoom [<pane_id>|--pane ID|--current] [--toggle|--on|--off]");
    eprintln!("  gardn pane rename <pane_id> <label>|--clear");
    eprintln!("  gardn pane read <pane_id> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!(
        "  gardn pane split [<pane_id>|--pane ID|--current] --direction right|down [--ratio FLOAT] [--cwd PATH] [--host EXECUTION_HOST_ID] [--env KEY=VALUE] [--focus] [--no-focus]"
    );
    eprintln!("  gardn pane swap --direction left|right|up|down [--pane ID|--current]");
    eprintln!("  gardn pane swap --source-pane ID --target-pane ID");
    eprintln!("  gardn pane move <pane_id> --tab <tab_id> --split right|down [--target-pane ID] [--ratio FLOAT] [--focus|--no-focus]");
    eprintln!(
        "  gardn pane move <pane_id> --new-tab [--workspace ID] [--label TEXT] [--focus|--no-focus]"
    );
    eprintln!("  gardn pane move <pane_id> --new-workspace [--label TEXT] [--tab-label TEXT] [--focus|--no-focus]");
    ...
    eprintln!("  gardn pane run <pane_id> <command>");
}
...
mod tests {
    ...
    fn parse_pane_swap_args_rejects_mixed_forms() {
        ...
        assert!(err.contains("usage: gardn pane swap"));
    }
    ...
    fn parse_pane_move_args_rejects_target_pane_without_tab() {
        ...
        assert!(err.contains("usage: gardn pane move"));
    }
