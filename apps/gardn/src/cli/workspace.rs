...
fn workspace_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: gardn workspace list");
        return Ok(2);
    }
...
fn workspace_get(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: gardn workspace get <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn workspace get <workspace_id>");
...
fn workspace_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: gardn workspace focus <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn workspace focus <workspace_id>");
...
fn workspace_rename(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: gardn workspace rename <workspace_id> <label>");
        return Ok(2);
    }
...
fn workspace_close(args: &[String]) -> std::io::Result<i32> {
    let Some(raw_workspace_id) = args.first() else {
        eprintln!("usage: gardn workspace close <workspace_id>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gardn workspace close <workspace_id>");
...
fn print_workspace_help() {
    eprintln!("gardn workspace commands:");
    eprintln!("  gardn workspace list");
    eprintln!("  gardn workspace create [--cwd PATH] [--host EXECUTION_HOST_ID] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]");
    eprintln!("  gardn workspace get <workspace_id>");
    eprintln!("  gardn workspace focus <workspace_id>");
    eprintln!("  gardn workspace rename <workspace_id> <label>");
    eprintln!("  gardn workspace close <workspace_id>");
}
