...
fn api_snapshot(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: gardn api snapshot");
        return Ok(2);
    }
...
fn schema_summary_text() -> std::io::Result<String> {
    ...
    Ok(format!(
        "Gardn API schema\nprotocol: {protocol}\nschema_version: {schema_version}\nschemas: {}\n\nUse `gardn api schema --json` to print the full schema.\nUse `gardn api schema --output PATH` to write it to a file.\n",
        schemas.join(", ")
    ))
}

fn print_api_help() {
    eprintln!("gardn api commands:");
    eprintln!("  gardn api snapshot");
    eprintln!("  gardn api schema [--json | --output PATH]");
}

fn print_api_schema_help() {
    eprintln!("usage: gardn api schema [--json | --output PATH]");
}
...
mod tests {
    ...
    fn schema_summary_text_stays_human_sized() {
        ...
        assert!(text.contains("Gardn API schema"));
        assert!(text.contains("Use `gardn api schema --json`"));
        assert!(text.len() < 400);
    }
}
