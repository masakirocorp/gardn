...
pub(super) fn mismatch_response(
    ...
    Some(ErrorResponse {
        ...
        error: ErrorBody {
            ...
            message: format!(
                "Gardn CLI protocol {client_protocol} is incompatible with server protocol {server_protocol} (server version {server_version}). Update and restart Gardn so the CLI and server use the same release."
            ),
        },
    })
}
...
mod tests {
    ...
    fn mismatch_preserves_request_id_and_reports_both_protocols() {
        ...
        assert!(response.error.message.contains("Update and restart Gardn"));
    }
