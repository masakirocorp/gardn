...
fn agent_explain(args: &[String]) -> std::io::Result<i32> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!("usage: gardn agent explain <target> [--json|--verbose]");
                eprintln!(
                    "usage: gardn agent explain --file PATH --agent LABEL [--json|--verbose]"
                );
                return Ok(0);
            }
...
fn agent_start(args: &[String]) -> std::io::Result<i32> {
    let Some(name) = args.first() else {
        eprintln!("usage: gardn agent start <name> [--cwd PATH] [--workspace ID] [--tab ID] [--split right|down] [--env KEY=VALUE] [--focus|--no-focus] -- <argv...>");
        return Ok(2);
    };
...
fn agent_attach(args: &[String]) -> std::io::Result<i32> {
    let (target, takeover) =
        match super::parse_attach_target(args, "usage: gardn agent attach <target> [--takeover]") {
            Ok(parsed) => parsed,
...
fn agent_send(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: gardn agent send <target> <text>");
        return Ok(2);
    }
...
fn print_agent_help() {
    ...
    eprintln!("  gardn agent send <target> <text>");
    ...
    eprintln!("  gardn agent start <name> [--cwd PATH] [--workspace ID] [--tab ID] [--split right|down] [--env KEY=VALUE] [--focus|--no-focus] -- <argv...>");
