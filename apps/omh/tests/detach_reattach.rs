...
use support::{
    ...
    register_runtime_dir, register_spawned_omh_pid, send_detach, send_input,
    unregister_spawned_omh_pid, wait_for_file, wait_for_socket, wait_until,
};
...
#[test]
fn explicit_detach_message_keeps_server_available_for_new_clients() {
    ...
    let mut reattached = connect_unix_socket(&client_socket, Duration::from_secs(5));
    let (version, error) =
        client_handshake(&mut reattached, 12, 80, 24).expect("reattach handshake should succeed");
    assert_eq!(version, 12);
    assert!(error.is_none(), "{:?}", error);
