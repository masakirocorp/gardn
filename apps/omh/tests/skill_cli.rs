...
fn unique_socket_path() -> PathBuf {
    let stamp = SystemTime::now()
        ...
        .as_nanos();
    PathBuf::from(format!(
        "/tmp/omh-skill-cli-{stamp}-{:x}.sock",
        std::process::id()
    ))
}

fn run_omh(args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_omh"))
        .args(args)
        .env("OMH_SOCKET_PATH", unique_socket_path())
        .output()
        .expect("omh CLI should start")
}
...
fn skill_flag_prints_bundled_agent_skill_and_exits() {
    let output = run_omh(&["--skill"]);
    assert!(
        output.status.success(),
        "omh --skill failed: status={:?} stderr={}",
        output.status.code(),
        stderr(&output)
    );
    ...
    assert!(
        printed.contains("# Oh My Herdr — agent skill"),
        "printed skill lost Oh My Herdr identity: {printed}"
    );
    assert!(
        !printed.contains("herdr.dev") && !printed.to_lowercase().contains("herdr --skill"),
        "printed skill must not use Herdr branding: {printed}"
    );
    assert!(
        stderr(&output).is_empty(),
        "omh --skill should write only to stdout: {}",
        stderr(&output)
    );
}
...
fn root_help_documents_skill_flag_and_next_cli_step() {
    ...
    assert!(
        help.contains("omh --skill prints agent instructions for driving omh from a pane"),
        "root help is missing the skill next-step hint: {help}"
    );
    assert!(
        !help.contains("herdr --skill") && !help.contains("https://herdr.dev"),
        "root help must not use Herdr branding: {help}"
    );
}

#[test]
fn next_step_hints_render_without_replacing_existing_help() {
    let agent_start = run_omh(&["agent", "start", "--help"]);
    assert!(
    ...
    assert!(
        agent_start_help.contains("next: omh agent prompt <TARGET> <TEXT> --wait"),
        "agent start is missing its next-step hint: {agent_start_help}"
    );
    assert!(
        !agent_start_help.contains("herdr agent"),
        "agent start help must use omh naming: {agent_start_help}"
    );

    let pane_send_text = run_omh(&["pane", "send-text", "--help"]);
    assert!(
    ...
    assert!(
        pane_send_text_help.contains("usage: omh pane send-text <pane_id> <text>"),
        "pane send-text dropped its existing usage: {pane_send_text_help}"
    );
    assert!(
        pane_send_text_help
            .contains("next: omh pane run <PANE_ID> <COMMAND> sends text and Enter in one call"),
        "pane send-text is missing its next-step hint: {pane_send_text_help}"
    );
    assert!(
        !pane_send_text_help.contains("herdr pane"),
        "pane send-text help must use omh naming: {pane_send_text_help}"
    );
}
