...
const PANE_RUNTIME_MARKER_ENV_VAR: &str = "OMH_PANE_RUNTIME_ID";
...
pub fn show_desktop_notification(title: &str, body: Option<&str>) -> std::io::Result<bool> {
    ...
    std::thread::Builder::new()
        .name("omh-windows-notification".into())
...
fn show_desktop_notification_on_thread(
    ...
    let window_name = wide_null("Oh My Herdr notifications");
    let hwnd = unsafe {
        CreateWindowExW(
            0,
            class_name.as_ptr(),
    ...
    copy_wide_truncated(&mut notification.szTip, "Oh My Herdr");

    if unsafe { Shell_NotifyIconW(NIM_ADD, &notification) } == 0 {
        let _ = ready_tx.send(Err(std::io::Error::other(
            "failed to add Oh My Herdr notification-area icon",
        )));
        unsafe {
            DestroyWindow(hwnd);
        }
    ...
    if unsafe { Shell_NotifyIconW(NIM_MODIFY, &notification) } == 0 {
        ...
        let _ = ready_tx.send(Err(std::io::Error::other(
            "failed to show Oh My Herdr desktop notification",
        )));
        return;
    }
...
//
// Windows IMEs live in the terminal-emulator process, not in Oh My Herdr. Empirically:
...
/// `SendMessageW` would block Oh My Herdr's client thread until that process responds
...
mod tests {
    ...
    const CONSOLE_TEST_PARENT_PID_ENV: &str = "OMH_TEST_CONSOLE_PARENT_PID";
    const WMI_DAEMON_TEST_CHILD_ENV: &str = "OMH_TEST_WMI_DAEMON_CHILD";
