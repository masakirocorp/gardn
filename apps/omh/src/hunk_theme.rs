...
use crate::external_tool_theme::{background_fallback, blend, foreground_fallback, palette_color};
#[cfg(test)]
...
pub(crate) fn command(
  ...
  config_dir="$(mktemp -d "${{TMPDIR:-/tmp}}/omh-hunk.XXXXXX")" || exec hunk diff --watch --theme graphite
  cleanup() {{
    rm -rf "$config_dir"
  }}
  trap cleanup EXIT INT TERM
  mkdir -p "$config_dir/hunk" || exec hunk diff --watch --theme graphite
  cat > "$config_dir/hunk/config.toml" <<'OMH_HUNK_CONFIG'
{config}
OMH_HUNK_CONFIG
...
mod tests {
    ...
    #[test]
    fn hunk_command_uses_oh_my_herdr_theme_for_hunk_014_and_falls_back_for_old_hunk() {
        let command = command(
            &Palette::tokyo_night(),
        ...
        assert!(command.contains("label = \"Oh My Herdr\""));
    ...
    #[test]
    fn oh_my_herdr_accent_drives_prominent_hunk_accent_fields() {
        let config = config(
            &Palette::tokyo_night(),
            ThemeAppearance::Dark,
            TerminalTheme::default(),
