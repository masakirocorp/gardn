    ...
    D: serde::Deserializer<'de>,
{
    ...
    for (id, rows) in &rows_by_agent {
        if crate::detect::parse_agent_label(id)
            .is_none_or(|agent| crate::detect::agent_label(agent) != id)
        {
            return Err(serde::de::Error::custom(format!(
...
mod tests {
    ...
    #[test]
    fn sidebar_token_defaults_parse_custom_and_reject_invalid() {
        let config: crate::config::Config = toml::from_str(
            r#"
[ui.sidebar.agents]
rows = [["state_icon", "workspace"], ["agent", "$summary"]]
...
"#,
        ...
        assert_eq!(
            config.ui.sidebar.agents.rows[1][1],
            AgentSidebarToken::Custom("summary".into())
        );
        assert_eq!(
            config.ui.sidebar.agents.rows_by_agent["claude"][0][0],
            AgentSidebarToken::TerminalTitleStripped
        );
        assert_eq!(
            config.ui.sidebar.spaces.rows[1][0],
            SpaceSidebarToken::Custom("jj_status".into())
        );
    ...
    #[test]
    fn sidebar_token_layout_limits_are_enforced() {
