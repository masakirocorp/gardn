...
impl ConnectionRetirementCoordinatorTransport for LocalCoordinatorTransport {
    ...
    fn start_coordinator(
        ...
        let exe = std::env::current_exe().map_err(|error| {
            session_error(
                &session.session_name,
                format!("failed to resolve current omh binary: {error}"),
            )
        })?;
        ...
        command.env_remove("OMH_CLIENT_SOCKET_PATH");
...
fn session_socket_path(session: &SessionRetirementImpact) -> PathBuf {
    session
        .snapshot_path
        .parent()
        .map(|dir| dir.join("omh.sock"))
        .unwrap_or_else(|| PathBuf::from("omh.sock"))
}
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        let unique = format!(
            "omh-connection-retirement-runner-{}-{}-{}",
            label,
            std::process::id(),
            SystemTime::now()
    ...
    fn finalizer_runs_only_after_every_session_is_ready() {
        ...
        let running_socket = root.join("running").join("omh.sock");
        let dormant_socket = root.join("dormant").join("omh.sock");
    ...
    fn unreachable_running_session_fails_closed_without_finalizer() {
        ...
        let socket_path = root.join("omh.sock");
