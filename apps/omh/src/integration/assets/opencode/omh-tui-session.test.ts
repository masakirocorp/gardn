...
beforeEach(() => {
  ...
  process.env.OMH_SOCKET_PATH = "test.sock";
...
async function loadPlugin() {
  importCounter += 1;
  const module = await import(`./omh-tui-session.js?test=${importCounter}`);
...
test("retries an initial selection while Oh My Herdr detects the process", async () => {
  const plugin = await loadPlugin();
  const tui = fakeApi();
  tui.addSession({ id: "session-a" });
  tui.select("session-a");
