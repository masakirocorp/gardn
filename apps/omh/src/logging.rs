...
pub(crate) fn help_log_paths_summary() -> String {
    ...
    format!(
        "{} (plus omh-client.log, omh-server.log)",
        dir.join("omh.log").display()
    )
}
...
mod tests {
    ...
    fn temp_log_path(name: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(unique).join("omh.log")
    }

    #[test]
    fn rotated_log_path_appends_numeric_suffix() {
        let path = PathBuf::from("/tmp/omh.log");
        assert_eq!(rotated_log_path(&path, 2), PathBuf::from("/tmp/omh.log.2"));
    }
    ...
    fn write_replaces_log_without_retained_files_when_size_limit_is_reached() {
        ...
        let writer = RotatingFileMakeWriter::new(dir.clone(), "omh.log", 8, 0).unwrap();
        {
