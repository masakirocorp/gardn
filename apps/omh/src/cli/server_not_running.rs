...
/// `ReportedProtocolMismatch`, except printing is deferred because several CLI
...
pub(super) fn response(request_id: &str, socket_path: &Path) -> ErrorResponse {
    ...
    ErrorResponse {
        ...
        error: ErrorBody {
            ...
            message: format!(
                "no Oh My Herdr server is running at {}; run `{attach_command}` to start or attach it",
                socket_path.display()
            ),
        },
    }
...
fn startup_command(socket_path: &Path) -> String {
    ...
    if socket_path == session_socket {
        ...
        // A socket override wins over an inherited OMH_SESSION. Keep the
