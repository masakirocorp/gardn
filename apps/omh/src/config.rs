...
pub use self::{
    ...
    model::{
        validated_sidebar_bounds, AgentPanelScopeConfig, CommandsConfig, Config,
        ConfigReloadReport, ConfigReloadStatus, ContextBarVisibilityConfig, HostCursorModeConfig,
        NewTerminalCwdConfig, PaneBorderAgentInfoConfig, ShellModeConfig, SidebarArrangementConfig,
...
mod tests {
    ...
    #[test]
    fn collect_diagnostics_includes_unknown_theme_and_invalid_headless_size() {
...
"#,
        ...
        assert!(diagnostics
            .iter()
            .any(|diag| diag.contains("unknown theme name theme.name = \"not-a-theme\"")));
        assert!(diagnostics
            .iter()
            .any(|diag| { diag.contains("server.headless_cols") && diag.contains("got 0x24") }));
    }
...
"#,
        ...
        assert!(!profile.contains("[[keys.command]]"));
        assert!(!profile.contains("switch_tab = \"prefix+1..0\""));
        assert!(profile.contains("\"prefix+2\""));
        assert!(profile.contains("\"prefix+9\""));
        assert!(profile.contains("\"prefix+0\""));
    ...
    #[test]
    fn local_keybindings_profile_preserves_effective_user_indexed_subset() {
        let config: Config = toml::from_str(
            r#"
[keys]
...
"#,
        ...
        let profile = config.local_keybindings_profile_toml().unwrap();
        let round_tripped: Config = toml::from_str(&profile).unwrap();
        let switch_tab_labels: Vec<_> = round_tripped
            .keybinds()
            .switch_tab
            .iter()
        ...
        assert!(!profile.contains("[[keys.command]]"));
        assert!(!switch_tab_labels.contains(&"prefix+1".to_string()));
        assert!(switch_tab_labels.contains(&"prefix+2".to_string()));
        assert!(switch_tab_labels.contains(&"prefix+0".to_string()));
        assert_eq!(switch_tab_labels.len(), 9);
    }

    #[test]
    fn local_keybindings_profile_preserves_effective_legacy_indexed_subset() {
        let config: Config = toml::from_str(
            r#"
[keys.indexed]
tabs = "ctrl"
...
"#,
        ...
        assert!(!profile.contains("[[keys.command]]"));
        assert!(!switch_tab_labels.contains(&"ctrl+1".to_string()));
