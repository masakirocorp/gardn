//! Windows thin-client launcher that attaches over SSH to a Unix Oh My Herdr host.
...
fn remote_omh_from_path(remote_omh: &RemoteOmh, path: &str) -> Option<RemoteOmh> {
    ...
    Some(remote_omh.clone().with_shell_path(posix_shell_quote(path)))
}
...
if ! mkdir "$lock" 2>/dev/null; then
  printf '%s\n' "another Oh My Herdr worker install is publishing $dest" >&2
  exit 75
fi
cleanup() {{
  rm -f -- "$tmp"
...
mod tests {
    ...
    fn extract_remote_args_requires_remote_for_keybindings() {
        let args = vec!["omh".into(), "--remote-keybindings".into(), "server".into()];
        let err = extract_remote_args(&args).unwrap_err();
    ...
    fn windows_reattach_command_uses_current_executable() {
        ...
        assert_eq!(
            reattach_command(
                r"C:\Program Files\Oh My Herdr\omh.exe",
                "host'name",
                "work'name",
    ...
    fn windows_local_forward_endpoint_uses_private_state_dir() {
        ...
        assert!(path
            .file_name()
            .is_some_and(|name| name.to_string_lossy().starts_with("omh-r-")));
    }
