...
impl Runtime {
    ...
    pub(crate) fn can_add_slot(&self, key: &Key) -> bool {
        self.slots.contains_key(key)
            || self.slots.len() < crate::api::schema::PANE_GRAPHICS_MAX_LAYERS_TOTAL
    }
    ...
    pub(crate) fn can_store_inline(&self, key: &Key, len: usize) -> bool {
        ...
        self.inline_bytes()
            ...
            .saturating_add(len)
            <= crate::api::schema::PANE_GRAPHICS_MAX_INLINE_BYTES_TOTAL
    }
...
impl Layer {
    ...
    pub(crate) fn data_len(&self) -> usize {
        match &self.backing {
            ...
            Backing::Resident { len, client_id } => {
                let _ = client_id;
                *len
            }
        }
...
mod tests {
    ...
    fn runtime_bounds_total_slots_and_inline_bytes() {
        let mut runtime = Runtime::default();
        for index in 0..crate::api::schema::PANE_GRAPHICS_MAX_LAYERS_TOTAL {
