...
Use `omh agent start`, `prompt`, `send-keys`, and `wait` to launch and drive a named agent without changing the pane layout unexpectedly. Prompt waits follow the same agent and pane through real status transitions, stop cleanly on disconnect, and report stalled or ended agents explicitly.
...
Oh My Herdr now reports startup configuration problems once, keeps them accessible from the bottom-left status menu, and provides a numbered diagnostics modal with reload, close, and click-outside dismissal.
...
Oh My Herdr now detects Maki panes as working, idle, or blocked and supports per-agent sound overrides for Maki notifications.

### Launch the Oh My Herdr product website
...
The source tree now uses a pnpm/Turborepo workspace with the Oh My Herdr app in `apps/omh` and separate docs and Nix release-note scopes.
...
Added `omh config check`, more tolerant graceful shutdowns, safer custom-command child reaping, complete initial API request reads, and stronger local connection handling across supported platforms.
