...
Gardn uses a shared integration checkout for final landing and isolated task worktrees for bigger, risky, parallel, or dirty-shared-checkout work. Read-only investigation can happen in the shared checkout. Small linear changes may happen in the default main worktree when the working tree is clean and no unrelated implementation is in progress.

Worktrunk (`wt`) is the task-worktree tool for Gardn. It owns worktree creation, switching, listing, merging, and removal. Worktrunk's configured path template owns task worktree locations. Task branches use `<tracker-key>-<slug>` when a tracker ticket exists.
...
Historical rationale beyond the current repository instructions is `[INFERENCE]`: this policy likely exists because Gardn is often changed by multiple agent sessions, and filesystem-level isolation is more reliable than relying on chat context to prevent accidental overlap.
