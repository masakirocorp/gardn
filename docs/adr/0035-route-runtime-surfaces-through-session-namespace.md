...
Oh My Herdr treats the active session name as the namespace for runtime-adjacent data surfaces. Parsing `--session` or `omh session attach <name>` happens before normal command dispatch, and the chosen session re-roots the data directory, Local API socket, client socket, logs, handoff import socket, and status reporting.
