...
Render delivery is intentionally freshness-biased. Client writers have a non-droppable prioritized control channel for shutdown, notifications, clipboard writes, and other control messages, plus a separate capacity-one render channel. `render_and_stream` uses `try_send` for render frames: if a client's render slot is full, Oh My Herdr marks that client `render_pending` and keeps the server loop moving instead of buffering old frames. When the writer drains the slot, the server event loop can render again and send the latest state.

Oversized graphics follow the same freshness rule. If a frame's graphics bytes exceed the graphics frame limit, Oh My Herdr drops graphics for that client frame and does not commit the graphics cache. If serialization is oversized only because of graphics, Oh My Herdr retries as text-only. A naturally oversized no-graphics frame is skipped without committing the render baseline. If the text-only retry itself cannot be serialized, Oh My Herdr treats that client as broken rather than blocking the server or corrupting the client's render baseline.
...
`[INFERENCE]` Oh My Herdr keeps render state per client because attached clients can have different sizes, encodings, host graphics state, and latency. A reliable replay queue would preserve old frames at the cost of making slow clients lag behind interactive state; a freshness-biased one-slot render channel keeps the server responsive and lets slow clients converge on the newest frame when they drain.
