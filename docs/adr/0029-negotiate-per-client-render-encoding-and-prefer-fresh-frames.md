...
`[INFERENCE]` Hako keeps render state per client because attached clients can have different sizes, encodings, host graphics state, and latency. A reliable replay queue would preserve old frames at the cost of making slow clients lag behind interactive state; a freshness-biased one-slot render channel keeps the server responsive and lets slow clients converge on the newest frame when they drain.
