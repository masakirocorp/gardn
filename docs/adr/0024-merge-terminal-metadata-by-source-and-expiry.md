...
`[INFERENCE]` Oh My Herdr merges metadata by source because multiple integrations can know different presentation facts about the same terminal. Replacing all presentation state globally on every report would let a narrow update, such as a custom status change, erase title or state-label data from another source.
