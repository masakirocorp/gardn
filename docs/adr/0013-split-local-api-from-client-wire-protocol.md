...
Socket path changes must preserve accepted API/client coupling. If `GARDN_SOCKET_PATH=/tmp/gardn.sock` is accepted as the active API socket path, the client protocol socket is `/tmp/gardn-client.sock`; explicit session selection overrides both. `GARDN_CLIENT_SOCKET_PATH` remains a legacy/client-only fallback when no API socket override is set.
