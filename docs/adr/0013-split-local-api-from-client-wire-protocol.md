...
Socket path changes must preserve accepted API/client coupling. If `OMH_SOCKET_PATH=/tmp/omh.sock` is accepted as the active API socket path, the client protocol socket is `/tmp/omh-client.sock`; explicit session selection overrides both. `OMH_CLIENT_SOCKET_PATH` remains a legacy/client-only fallback when no API socket override is set.
