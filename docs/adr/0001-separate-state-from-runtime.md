...
- Keep pane-local UI state, terminal metadata, and live runtime as one pane object. Rejected because Oh My Herdr needs stable terminal identity and metadata without making every workspace mutation depend on live terminal resources.
...
Historical rationale beyond the current source and repository instructions is `[INFERENCE]`: this boundary likely exists to keep Oh My Herdr's core workspace model deterministic while allowing terminal/runtime behavior to remain asynchronous and platform-sensitive.
