...
Historical rationale beyond the current source is `[INFERENCE]`: this split likely exists because Gardn needs durable restart files that remain stable and inspectable, while live handoff needs richer transient agent/runtime presentation to make update or server replacement feel continuous.
