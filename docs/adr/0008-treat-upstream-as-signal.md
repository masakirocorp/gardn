...
- Treat upstream as authority and keep Gardn close to Gardn by default. Rejected because it can resurrect upstream identity, docs, website, release plumbing, or behavior that conflicts with Gardn's product direction.
...
Historical rationale beyond the current repository instructions is `[INFERENCE]`: this policy likely exists because Gardn intentionally inherited useful Gardn code while becoming a separate product whose name, release line, update channel, docs, website, and operational workflow must not be silently overwritten by future upstream syncs.
