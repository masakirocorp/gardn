...
Future worktree behavior should keep linked-worktree removal and dirty-checkout safety separate from source selection. A linked checkout can be a target for open, and a Hako-managed linked workspace can be a target for remove, but neither should become the authoritative source for creating or listing the repository's worktree family.
