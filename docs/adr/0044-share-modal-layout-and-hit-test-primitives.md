...
`[INFERENCE]` Hako shares modal primitives because it is a mouse-first TUI: the same rectangle math must drive painting, hover, click, scroll, and selection behavior. Centralizing that geometry makes new overlays feel native and prevents one-off modal code from reintroducing inconsistent affordances.
