...
Gardn separates view computation from drawing. `compute_view*` functions take `&mut AppState` and may reconcile view geometry, scroll bounds, pane sizes, mobile layout, hit areas, and cached `ViewState`; `render*` functions take `&AppState` and must not mutate app, workspace, or layout state.
