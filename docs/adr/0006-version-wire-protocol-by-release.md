...
When changing `apps/hako/src/protocol/wire.rs`, presentation encoding under `apps/hako/src/protocol/`, framing, handshake fields, frame-size limits, or server/client behavior that would make an older Hako release client or server misdecode, reject, or semantically misinterpret the stream, compare current source `apps/hako/src/protocol/wire.rs::PROTOCOL_VERSION` (re-exported as `crate::protocol::PROTOCOL_VERSION`) against the latest Hako release tag. If current source is not already greater than the latest released protocol, bump it once and update all hardcoded expectations and manual protocol fixtures that intentionally pin the value.
