...
`[INFERENCE]` Hako restores stable workspace shells because session files are user-facing continuity, not strict crash dumps. Dropping an entire workspace because one cwd vanished or one legacy layout needs migration would lose names, grouping, and workflow context that are still valid.
