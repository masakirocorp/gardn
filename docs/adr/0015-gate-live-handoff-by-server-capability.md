...
If handoff fails after the binary is installed, Gardn classifies the post-failure server state. If the updated server is already running, the update is treated as complete. If the old server is still running, Gardn asks whether to stop it or keeps it alive; non-interactive updates keep it alive. If no server responds, Gardn prints session attach/restart guidance. If state cannot be determined, Gardn prints reconnect/stop guidance instead of pretending the live transition succeeded.
