...
If handoff fails after the binary is installed, Oh My Herdr classifies the post-failure server state. If the updated server is already running, the update is treated as complete. If the old server is still running, Oh My Herdr asks whether to stop it or keeps it alive; non-interactive updates keep it alive. If no server responds, Oh My Herdr prints session attach/restart guidance. If state cannot be determined, Oh My Herdr prints reconnect/stop guidance instead of pretending the live transition succeeded.
