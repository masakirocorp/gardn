...
The terminal-core bridge keeps both sides of a shifted character when Hako builds a Ghostty key event. `ghostty_key_event_from_terminal_key` sets Ghostty action from the semantic key kind, text from the shifted codepoint when available, and the unshifted codepoint from the base character. Character-key delivery can also prefer Hako text encoding before Ghostty key-event encoding, so shifted text may be delivered as text rather than as a round-trip key identity; shifted-text release events can intentionally emit no bytes.
...
`[INFERENCE]` Hako keeps legacy/xterm output for special keys because terminal applications have decades of compatibility with those sequences, while CSI u support is uneven. Using CSI u only where it buys information avoids breaking applications that already understand modified arrows, function keys, and navigation keys.
...
New output protocols should be conservative about replacing legacy sequences. If a key already has a broadly compatible xterm/legacy encoding, Hako should keep that encoding unless there is a concrete reason to require a richer protocol.
