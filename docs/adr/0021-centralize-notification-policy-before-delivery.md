...
`[INFERENCE]` Oh My Herdr centralizes notification policy because agent state can arrive through multiple paths such as fallback screen detection, hook/integration reports, and `pane.report_agent`. If each delivery path decided independently, active-tab handling, sound policy, notification text, and focus behavior would drift.

`[INFERENCE]` Oh My Herdr keeps terminal/system delivery best-effort because those channels depend on host terminal, foreground client, desktop platform, and user environment support. Treating delivery failure as a state change would make agent state less reliable than the delivery backend.
...
New delivery channels should preserve the same state-transition policy and channel-specific active-tab handling. If they cannot support `ToastTarget`, they should degrade like terminal/system delivery rather than weakening the in-app Oh My Herdr toast target contract.
