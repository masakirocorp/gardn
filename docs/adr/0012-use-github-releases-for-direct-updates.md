...
Oh My Herdr direct installs update from `masakirocorp/oh-my-herdr` GitHub Releases. `just release` is the release entry point: it requires a clean tree, verifies every pending Tegami changefile includes `omh`, runs `CI=true pnpm tegami version`, runs `just check`, commits Tegami's version/changelog changes, tags `v<version>`, and pushes the branch and tag. The `Release` GitHub Actions workflow runs on `v*` tags, verifies the tag matches `apps/omh/Cargo.toml`, builds the supported binary release assets, and publishes them on the GitHub Release.
