...
On macOS, Gardn.app owns updates through Sparkle when it is present. Direct Install `gardn update` is for CLI-only machines and refuses for the stable Direct CLI when Gardn.app is installed. `gardn-beta`, `gardn-dev`, mise, and Nix keep their own update owners.
