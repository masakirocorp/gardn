...
Release tags, `Cargo.toml` versions, and asset names are part of Hako's update contract. If a release misses `hako-linux-x86_64`, `hako-linux-aarch64`, `hako-macos-x86_64`, or `hako-macos-aarch64`, supported direct installs on that platform cannot update through `hako update`. If the latest GitHub Release tag does not parse as a Hako version, the direct update check fails instead of guessing.
