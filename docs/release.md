...
The automated workflows own the release gate. Before tagging, confirm that the source candidate's
CI and Nix workflows passed for the exact candidate SHA. `just release` also runs `pnpm check`
before it creates the release commit and tag.

Use the [manual QA matrix](manual-qa.md) only for additional risk-based checks. Manual QA does not
block tagging or publication.

The release commit generated by Tegami changes version and changelog files after the source checks.
The tag-triggered Release workflow reruns deterministic CI and Agent Fixture Tests at the exact tag
...
The local reusable workflow calls do not inherit secrets. A trusted canary remains optional operator evidence and is not a `release.yml` dependency.
...
Use M09 from the [manual QA matrix](manual-qa.md) when additional downloaded-artifact evidence is useful. M09 does not block release publication.
