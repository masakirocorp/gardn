...
The GitHub Release workflow builds binary assets from the pushed tag and uses the generated `apps/gardn/CHANGELOG.md` section as the release body. macOS also gets a signed, notarized `Gardn.app` disk image and a Sparkle update zip from `scripts/build-macos-app.sh`. Apple Developer ID and notary credentials are organization secrets. Sparkle signing uses `SPARKLE_ED25519_PRIVATE_KEY` on this repository.
