...
- `hako --remote` now offers to restart the remote server after installing or replacing a remote binary, or when the running server version differs, even if the client/server protocol is still compatible.
...
- Added `hako config reset-keys` to back up `config.toml` and remove custom keybindings so built-in v2 defaults apply on restart or config reload. (#154)
- Added an integrations tab in settings and first-run onboarding so users can install recommended agent integrations from inside Hako.
...
- Added `terminal.default_shell` to choose the executable used for new interactive panes. When unset, Hako still falls back to `$SHELL`, then `/bin/sh`. (#196)
...
- Running `hako server` directly now prints socket and log paths and explains that normal TUI users should run `hako`.
- Kitty graphics virtual Unicode placeholders now render image placements instead of leaving placeholder cells behind. (#136)
- Clipboard image reads are now capped to Hako's image payload limit, preventing oversized local clipboard images from being read into memory.
- The install script now reads Hako's public latest-release manifest, so fresh installs use the same binary URLs as `hako update`.
...
- Remote clients now bridge local clipboard images into the remote pane by staging them as temporary image files and pasting the remote path, so Claude Code image paste works over `hako --remote`. (#205)
...
### Fixed
- The Claude Code integration no longer reports successful or failed post-tool hooks as `working`, and installing the updated integration removes Hako's deprecated post-tool hook entries from existing Claude settings. (#198)
...
### Added
- Added the `terminal` built-in theme, which uses the host terminal's ANSI palette for Hako UI colors. (#140, #146, thanks @babymastodon)
...
### Fixed
- Running the internal `hako client` command from inside Hako now respects the nested-launch guard, and the command is no longer advertised in root help. (#187)
- The Hako agent skill now refuses to claim pane ownership unless it is running inside Hako. (#152)
...
- Hako now preserves `session.json` symlinks when saving persistent session state. (#139, #147, thanks @cloudmanic)
...
- Remote attach now uses a matching `hako` already available on the remote `PATH` before installing a new copy. (#170)
...
- Added direct terminal attach with `hako agent attach <target>` and `hako terminal attach <terminal_id>`.
...
- Hako now redraws immediately when the outer terminal regains focus.
...
### Fixed
- PageUp and PageDown now scroll Hako pane scrollback for normal panes while still forwarding keys to full-screen or mouse-reporting apps.
- Enhanced tilde key sequences now parse correctly, improving compatibility with terminals that emit them.
- `hako integration install codex` now enables the current Codex `[features] hooks = true` flag and migrates the deprecated top-level `codex_hooks` flag.

### Breaking Changes
- `advanced.allow_nested` has moved to `experimental.allow_nested`; update configs that allow nested Hako launches.
...
### Added
- Added manual pane labels through `hako pane rename`, the `pane.rename` socket API, an optional `keys.rename_pane` binding, and the right-click pane menu.
...
- Added `hako integration status [--outdated-only]` so installed agent integrations can be checked for legacy or outdated versions.
...
### Added
- Added ANSI-formatted pane reads to the CLI and socket API with `hako pane read --format ansi` / `--ansi`, preserving colors and styles for visible and recent pane output.
...
- `hako update` now downloads and prepares the new binary before stopping a running server, reducing the chance of interrupting an active session when download or install preparation fails.

## [0.5.6] - 2026-05-09

### Added
- Added the `vesper` built-in theme. (#71, thanks @nexxeln)
- Added `hako --remote <ssh-target>`, so you can use Hako as a thin client for remote servers without SSHing in first. Hako connects over SSH, bootstraps a matching remote `hako` binary when needed, starts the remote server automatically, and streams an efficient terminal view back to your local terminal.
...
### Fixed
- OSC 8 hyperlinks emitted inside panes now remain clickable after Hako renders them, including titled markdown-style links.
...
### Added
- Added a mobile layout for narrow terminals, making it practical to SSH into your machine and run hako from your phone.
...
### Fixed
- Visible active-tab panes that finish while the outer terminal is unfocused are now marked as seen when you return to hako, preventing stale done/attention indicators.
...
### Added
- Added named persistent sessions, so you can keep separate hako environments for different projects or contexts while sharing the same global config. See the docs for the full session CLI. (#57, thanks @fbettag)
- Added `hako status`, `hako status server`, and `hako status client` to inspect the local client, running server, protocol compatibility, socket path, and whether a restart is needed.

### Changed
- Focused panes can now still alert you through terminal notifications when the hako terminal window is unfocused, so active work does not go quiet just because you switched to another app.
...
- `hako integration install` and `hako integration uninstall` now work without requiring a running hako server.
...
### Added
- Config can now be reloaded in the running app/server from the global menu or with `hako server reload-config`, applying safe live settings without restarting the persistent server.
...
### Added
- Toast notifications can now be delivered through the outer terminal as desktop notifications. Configure this with `ui.toast.delivery = "terminal"`; see the [configuration docs](https://hako.masakiro.com/docs/configuration/) for details.
- Hako now writes separate capped support logs for app, client, and server modes, making persistent-session issue reports easier to diagnose without unbounded log growth.
...
- In-app toast delivery now stays inside hako instead of also forwarding a terminal/desktop notification.

## [0.5.0] - 2026-04-21

### Breaking Changes Please Read
- hako now defaults to a persistent server/client session model. running `hako` starts or reattaches to a background session server instead of launching the old single-process UI.
- quitting the UI in default mode now detaches the current client and leaves the shared session running. use `hako server stop` to stop the background server explicitly.
- the old monolithic behavior is still available as an escape hatch with `hako --no-session`.

### Added
- Persistent sessions are now the default product behavior. You can detach and reattach without stopping pane processes.
- Added the thin client and headless server as first-class product components, including auto-detect launch, explicit `hako client`, and `hako server stop`.
...
### Breaking Changes Please Read
- The update flow changes in `0.4.11`. Hako no longer installs updates silently in the background. Starting with this release, hako only checks for updates and shows them in the UI. To install a new release, quit hako and then run `hako update` manually in your shell.
- This prepares the upcoming `0.5.0` persistence release. Hako is moving from the old single-binary update model toward a persistent server/client session model, so your workspace can keep running while clients attach, detach, and reconnect.
- The reason for this change is upgrade safety. Hako needs to stop the old running process cleanly before the new client/server model takes over, so manual update avoids mixed-version states during the transition.

### Added
- Hook-reported agent state can now use custom agent labels, so integrations are no longer limited to hako’s built-in agent names. Custom labels now flow through pane/workspace UI and the socket API anywhere agent names are shown.

## [0.4.10] - 2026-04-14

### Added
- Prefix mode now supports custom command keybindings via `[[keys.command]]`, so you can launch detached shell helpers or open temporary overlay panes from inside hako using the active workspace, tab, pane, and cwd context.
...
### Fixed
- Droid's primary-screen redraws no longer erase pane scrollback inside hako, while normal scrollback-clear behavior is preserved elsewhere.
...
### Added
- Themes can now set `panel_bg = "reset"` to let hako’s panel chrome inherit the host terminal background instead of painting an opaque panel fill. This also accepts the aliases `default`, `none`, and `transparent`.
...
### Added
- `hako workspace create` and `hako tab create` now support `--label`, so scripts and agents can name new workspaces and tabs immediately instead of creating them first and renaming them afterward.
- The global menu now includes a manual **reload keybinds** action, so you can apply `config.toml` keybinding changes without restarting hako.
- The socket API and CLI now expose a `done` agent status, including `hako wait agent-status --status done`, so automation can distinguish finished agent runs from panes that are merely idle.

### Changed
- Session state is now saved automatically with a debounce while you work, so recent workspace, tab, pane, and sidebar changes are preserved more reliably even if hako exits unexpectedly.
...
- `hako pane run ...` now uses the bracketed-paste-aware input path, improving compatibility with shells and terminal apps that expect pasted command text to arrive atomically.
...
- `hako pane run ...` now submits the full command atomically in one request, fixing cases where scripted commands did not reliably execute because the final Enter was sent separately.
...
### Major Changes
- Hako now uses a Ghostty-backed terminal engine as its pane runtime.
...
- Debug builds now use separate runtime/config paths from normal releases, which avoids local development sessions colliding with your main hako install.

### Fixed
- Starting a second hako instance against an active socket now fails fast with a clear error instead of clobbering the running session.
...
- Added first-class tab support to the local socket API and CLI wrappers, including `hako tab ...` commands and tab ids like `1:2` alongside workspace-scoped pane ids.
...
- Added a post-update release-notes screen so hako can explain what changed after an update is installed.
...
### Changed
- Mouse wheel handling now follows the tmux/Ghostty model more closely: fullscreen apps receive wheel input when they own scrolling, while hako keeps host scrollback for panes that are behaving like a normal terminal transcript.
- Pane scrollbars now only appear when hako has real host scrollback for that pane, instead of implying a host-managed scroll position for app-owned scrolling.

### Fixed
- Fixed Codex and pi panes becoming unscrollable in hako by preserving recoverable host history for top-anchored normal-screen output, without relying on alternate-screen scrollback retention.
- Fixed pane wheel routing so apps using mouse reporting or alternate-scroll behavior can receive scroll input directly instead of having hako always intercept it.
...
- Fixed alternate-screen scrollback handling so full-screen terminal apps can preserve recoverable history inside hako panes instead of losing rows that scroll off.
- Fixed Codex in hako panes losing transcript/history while running in alternate screen, so past output remains scrollable instead of disappearing as the session grows.
...
### Added
- Hako now checks for updates at startup and periodically while it stays open, so long-running sessions can still discover new releases without a restart cycle.
...
### Added
- Added a local Unix socket API for controlling running hako sessions, including workspace and pane management, pane reads, text/key input, pane splitting, and output waits.
...
- Added CLI wrappers on top of the socket API with `hako workspace ...`, `hako pane ...`, and `hako wait ...`, using compact public ids like `1` and `1-2` for scripting and agent orchestration.
- Added a settings popup with mouse support for changing themes, sound alerts, and toast notifications from inside hako.
...
- Hako now blocks nested launches by default when started inside a hako-managed pane; set `advanced.allow_nested = true` to opt back in.
...
### Documentation
- Refreshed the README, socket API reference, and reusable agent skill docs to better explain hako's agent multiplexer model and integration surface.
...
- Added configurable keybindings for all navigate mode actions: new workspace, rename workspace, close workspace, resize mode, and toggle sidebar. See the [configuration docs](https://hako.masakiro.com/docs/configuration/) for the full key reference.
...
### Fixed
- Improved foreground job detection on Linux and macOS so hako can recognize agents that run through wrapper processes or generic runtimes, including cases like Codex running under `node`.
