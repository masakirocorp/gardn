...
- **Git diff command** — the command palette can open a Git diff panel when the current context is inside a Git repository.
...
Hako ships installable integrations for agents that can report semantic state over the socket API.
...
Hako uses a hosted manifest for update checks and release metadata.
