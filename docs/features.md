...
- **Project commands** — **Settings > Commands** configures four independent project launchers: **Browser** (`terminal-browser`), **Review** (`hunk diff --watch`), **Editor** (`fresh .`), and **GitHub** (`ghui`). Each field is freely editable; leaving one empty hides only that action. All four appear in the command palette and workspace and `+` menus. Browser, Editor, and GitHub run from the workspace directory; Review uses observed Git repositories. The curated GitHub launcher requires the pinned Masakiro ghui companion release. Gardn passes the active terminal palette, visible scrollbars, Space name, and resolved GitHub scope as launch-only settings.
- **Space GitHub scope.** Space Settings offers Automatic, Selected repositories, and Group organization modes. Automatic uses GitHub repositories discovered from the Space's local directories. If it finds none, it uses the Group organization or the signed-in user's queues. Discovery errors remain visible. Selected repositories accepts one or more `owner/repository` identities and overrides the Group organization. Press Enter to save the repository list. The curated launcher requires a local Space with local panes. A running ghui keeps its launch scope when a shell changes directories or the Space settings change. Stop and restart it to use the new scope.
- **GitHub scope visibility.** ghui shows the active organization or repositories before the Space name in its header. Space Settings names the configured organization beside the Group organization option and separates the scope choices from the repository input.
- **GitHub handoffs.** ghui can focus a local repository Space or open a pull request in an isolated Worktrunk review Space. Review creation preserves the original checkout and refuses to reuse a changed review snapshot. Agent handoffs send repository, URL, commit, and status context to an explicitly selected agent in the same Gardn session.
...
| Configured project commands | Browser, review, editor, and GitHub actions from project contexts. Configure `[commands].browser`, `[commands].review`, `[commands].editor`, and `[commands].github`; defaults are `terminal-browser`, `hunk diff --watch`, `fresh .`, and `ghui`. | Each tool is optional; required only when its configured action is used. Install the pinned Masakiro ghui fork on macOS with `brew install masakirocorp/tap/ghui`. |
