...
Appearance > Panes exposes `Pane Borders`, `Pane Scrollbars`, `Pane Gaps`, `Hide Single-Tab Bar`, and `Pane Border Agent Info`. Behavior exposes `Name New Workspaces`, `Copy on Select`, and `Right-click Passthrough` alongside the existing close confirmation, tab naming, counters, new-terminal CWD, and mouse-wheel speed controls.
