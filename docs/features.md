...
- **macOS app** — Gardn.app is the macOS install. It owns `~/.local/bin/gardn` by linking it to the bundled CLI. The app's menu bar surface lists the same Triage, Follow Up, Working, and Idle groups as the Agents sidebar. It supports local sessions and saved remote Coordinator Hosts through `gardn extra connect --remote`. The standalone `gardn update` command refuses to update when Gardn.app is installed. Use Check for Updates in Gardn, or uninstall the app before using a standalone CLI update. The app ships a matching `gardn` binary for its own use. The menu bar leaf fills when Follow Up or Triage needs attention. Click a row to focus that agent in an attached client.
...
- **Group settings** — rename groups, choose a group icon, set the default location and directory for new spaces, assign per-group theme accent colors, choose favorite/default agent profiles, or inherit the global accent from the group settings modal. An SSH group default can be saved without typing a directory; Gardn uses the connection's suggested directory.
...
Direct installs use GitHub Releases for update checks, release metadata, and binary downloads on Linux, macOS, and Windows. mise and Nix-managed installs are routed to their package manager instead of self-update. On macOS, Gardn.app owns updates through Sparkle when it is installed. The standalone `gardn update` command is for CLI-only machines and refuses to update when Gardn.app is present.
