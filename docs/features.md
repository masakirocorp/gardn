...
- **Agent sidebar** — agent rows focus their workspace, tab, and pane when clicked and highlight the active agent row for the attached client. When multiple agents share a workspace label, rows append the tab name; agents sharing a tab append their pane label or number.
