...
- **Desktop context bar** — a bottom row shows the attached client's active group / workspace / tab path on the left and live topology counts on the right. `ui.context_bar = "auto"` hides the duplicate row while the spaces sidebar is expanded; use `"always"` or `"never"` for a fixed policy, or toggle one client temporarily with `prefix+Down`. Click a path segment to open the existing group menu or workspace navigator; narrow terminals drop counts before shortening the active path.
