...
- **Collapsed sidebar rail** — collapsed sidebars keep group boundaries and activity categories visible: group rows switch directly to that group's remembered workspace, space rows switch directly to a workspace, and activity counts open filtered agent pickers. Hovering or keyboard-selecting a compact space row reveals its full name.
