...
Appearance > Panes exposes `Pane Borders`, `Pane Scrollbars`, `Pane Gaps`, `Hide Single-Tab Bar`, and `Pane Border Agent Info`. Appearance > Window edits the outer terminal window-title template. Behavior exposes `Name New Workspaces`, `Copy on Select`, `Right-click Passthrough`, and `Resume Agent Sessions` alongside the existing close confirmation, tab naming, counters, new-terminal CWD, and mouse-wheel speed controls. Advanced > Server edits the fallback terminal columns and rows used when no client is attached.
