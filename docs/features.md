...
- **Workspace sidebar** — expanded workspace rows show the workspace name, activity state, and git/cwd summary. Connection badges use each SSH profile's optional accent color; mixed workspaces show every resolved connection, while connection health remains a separate status label.
- **Execution host labels and filters** — pane borders, Space rows, Runs On, and host pickers name the coordinator by its hostname (or `ui.coordinator_display_name`) and SSH hosts by connection profile name. The Space and Agent filter popovers have independent per-client connection filters that include the coordinator and every configured profile, including disconnected profiles. The filter menus identify the coordinator as `(local)` and connection profiles as `(ssh)`. Group and agent scopes remain independent and intersect their sidebar's selected connection.
...
- **New-client sidebar defaults** — every app client starts with all spaces and all connections visible. `ui.sidebar.initial_state` and `ui.sidebar.initial_agent_scope` choose its initial expansion and agent scope; defaults are `expanded` and `all`, and one client's runtime changes never seed another client.
...
- **Activity sidebar** — shows agents in Triage, Follow Up, Blocked, Working, and Idle order. Pending Blocked conditions appear in Triage; right-click **Mark Reviewed** moves only that condition to Blocked without changing focus, terminal input, process state, Done acknowledgement, or Follow Up membership. Follow Up has placement precedence, so queued Blocked agents stay there without losing their pending or reviewed state. An expanded empty Follow Up section shows a muted `Drop an agent here` row on desktop and mobile. Its full desktop width and the section header accept drops. Follow Up is shared placement, not a lifecycle state: drag an agent row onto the Follow Up header or body to queue it, or right-click any agent row to add it to or remove it from Follow Up. Queued agents keep their real runtime state and waiting age, ordered oldest-added first. Successful unmodified Enter from the human terminal input path clears that pane's placement; typing, paste, failed sends, and API automation do not. Closing a pane or restoring without that target drops stale queue entries. Triage lists oldest meaningful activity first (no activity counts as oldest); Blocked, Working, and Idle stay newest-first. The Local API's `in_triage` field uses the same placement policy.
- **macOS app** — Gardn.app is the macOS install. It owns `~/.local/bin/gardn` by linking it to the bundled CLI. An installed release sets toast delivery to system when neither `ui.toast.delivery` nor legacy `ui.toast.enabled` exists. It never replaces an explicit notification choice. The menu bar surface lists its Triage, Follow Up, Blocked, Working, and Idle groups from the Local API. The header selects which Gardn instance to observe: this Mac's default or named local session, or a saved standalone remote coordinator (`gardn extra connect --remote`). SSH Connections are execution hosts inside the selected instance and do not appear in this selector. The app refreshes the instance catalog before it displays the menu or Settings window. Settings in the extra header opens a window for Gardn instances and updates. Sparkle owns updates when the runtime supports it. GardnMenu registers as the native system-notification presenter and opens the stable terminal target when a notification is selected.
...
- **Native GitHub.** GitHub opens in a dedicated native tab named `GitHub` for the invoking client. The tab is durable session membership: it survives client detachment, disconnection, and session restart. Each client's GitHub screen and breadcrumb selection are transient. After reattach or restart, focusing the tab initializes them from the current Space scope. The source tab stays open, and only that client selects the new tab. Explicit GitHub Close or an ordinary tab close removes the dedicated tab and restores the source pane when it still exists. It uses the existing authenticated `gh` CLI and Gardn's palette, theme, scrollbars, and settings. There is no separate GitHub command, companion version, configuration, or theme to manage.
- **Space GitHub scope.** **Space Settings > GitHub** still provides the default scope. The native GitHub header now shows `GitHub / account / repository` as an interactive breadcrumb. Browse the Space default, personal queues, discovered organizations, all repositories, or one repository without changing saved settings. Narrow panes collapse the breadcrumb to one `Scope ▾` control while keeping the adaptive toolbar on its own row.
- **GitHub filters and hover.** GitHub list views use one adaptive control row. All five tabs remain visible, using full labels first, compact labels next, and numeric labels in the narrowest panes. Refresh and Filter stay visible when they fit. Queue, More, and other omitted commands move into the `…` menu without duplicating visible controls. Detail views use the same row for Back and detail navigation, with omitted navigation and selected-item commands in the menu. **Queue** opens ...
- **GitHub actions menu.** The `…` menu opens local commands for the current GitHub view or selected item. It does not open the global command palette. Use the mouse or arrow keys and Enter to choose an action. Escape or an outside click dismisses the menu. Long menus scroll in compact panes. Ctrl+P still opens the global command palette.
- **GitHub descriptions.** Pull request and issue details separate the title, status, and metadata from the description. Markdown headings, lists, checklists, emphasis, and code retain their formatting. Links show their labels instead of raw URLs. Click a web link to open it, or use Tab and Enter in the detail area. Empty labels and assignees are hidden.
- **GitHub workflows.** Overview summarizes scoped work. Pull requests and issues include conversations, comments, edits to your own comments, labels, and close actions. Pull requests also support draft state, split or unified diffs, wrapping, whitespace controls, file navigation, inline range reviews, and safe merge, auto-merge, or queue actions. Actions lists scoped workflow runs with jobs, steps, and log links. Text filters apply to loaded results. **More** loads another page.
