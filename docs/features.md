...
- **Execution host labels and filters** — pane borders, Space rows, Runs On, and host pickers name the coordinator by its hostname (or `ui.coordinator_display_name`) and SSH hosts by connection profile name. The Space and Agent filter popovers share a per-client connection filter that includes the coordinator and every configured profile, including disconnected profiles. Group and agent scopes remain independent and intersect the selected connection. The UI does not show Local as a host.
