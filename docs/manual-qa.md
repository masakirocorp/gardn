...
Use this guide to select and run the manual checks that Hako's automated suite cannot prove reliably. It complements `just check`; it does not repeat state, protocol, socket, PTY, or render behavior already covered by automated tests.
...
Record each selected case as `PASS`, `FAIL`, or `BLOCKED`. For failures, preserve the relevant Hako logs, exact reproduction steps, and a screenshot or short recording when presentation matters. Track each defect separately and link it from the run record.
...
1. Use an isolated `hako-dev` configuration or disposable OS user and a named QA session.
...
3. Verify Hako's working, blocked, idle, and release transitions. Verify child completion never idles or releases the parent and the pane is never labeled as another agent.
...
Pass when state matches the visible agent, identity remains stable, restore works, and install or uninstall changes only Hako-owned integration files.

## M08: Remote attach and bootstrap

1. Attach to a clean Linux host over SSH with no running Hako server and exercise bootstrap.
2. Create a pane workload, interrupt the SSH connection, and reconnect.
3. Repeat with an older remote Hako binary to exercise the compatibility and restart prompt.
