...
A linked plugin must still have a readable manifest at its registered path. A missing, unreadable, malformed, or incompatible manifest is reported as a manifest warning or error. Fix the source and re-link it, or unlink the entry before trying another source. The preferred filename is `gardn-plugin.toml`; `gardn-plugin.toml` is a compatibility alias only.
...
3. Run `gardn config check` if an Gardn keybinding was changed, then use the keybinding-help surface to confirm the active binding.
