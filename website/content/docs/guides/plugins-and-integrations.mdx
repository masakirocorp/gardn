...
The directory (or a direct manifest path) must contain a valid `gardn-plugin.toml`; the legacy `gardn-plugin.toml` filename is accepted only for compatibility. Use `--disabled` to register without running its actions or event hooks:
