...
Gardn provides four project actions. Each action opens in a managed tab and uses the selected workspace context.
...
| GitHub  | `ghui`              | `brew install masakirocorp/tap/ghui`                     |

To open a tool:

1. Press **Ctrl+B**, then **Space**.
2. Search for **Open Browser**, **Open Review**, **Open Editor**, or **Open GitHub**.
3. Select the action and press **Enter**.

Browser, Editor, and GitHub start in the workspace directory. Review starts in an observed Git repository. Gardn asks you to select a repository when the workspace contains more than one.
...
The curated GitHub launcher requires the pinned Masakiro ghui companion release. Gardn passes the active terminal palette and visible scrollbars as launch-only settings. Gardn does not modify your saved ghui configuration. See **Settings > About** for the upstream copyright and MIT acknowledgment.

To scope GitHub for a group:

1. Right-click the group and select **Settings**.
2. Enter the GitHub login in **GitHub Organization**.
3. Press **Enter** to save it.

Space Settings controls how each Space uses GitHub:
...
The curated launcher requires a local Space with local panes. A running ghui keeps its launch scope when directories or settings change. Stop and restart it to apply new settings.

Overview shows authored pull requests, requested reviews, and assigned issues. Actions lists workflow runs across the current scope. Click a run to inspect jobs and step logs. Click the scope header to narrow to one repository; HOME restores the launch scope. Explicit outside-scope views remain available and are marked.

Use ghui's command palette to open a local checkout, create an isolated Worktrunk review Space, or preview context for a selected Gardn agent. Review creation preserves the original checkout and refuses to reuse a changed or dirty review snapshot.
