...
Use this how-to when you are arranging an active Oh My Herdr session. For the complete shortcut list, see the [keybinding reference](/docs/reference/keybindings).
...
Press the default `ctrl+b`, then `w` to open the workspace navigator directly. Search by name or filter by state, use the disclosure arrows or `space` to expand or collapse the highlighted branch, press `e` to expand the full tree, or press `c` to collapse it to group roots.
