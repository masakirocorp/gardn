...
The prompt applies to interactive creation from the sidebar, menus, navigator, and context bar. `omh workspace create` and `workspace.create` remain non-interactive and use their own `--label` or `label` input.
