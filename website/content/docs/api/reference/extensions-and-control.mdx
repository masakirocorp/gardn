...
| `expected_version`  | `string \| null`  | `null`  | If present, the replacement must report this Gardn version.              |
...
These methods are client-facing. The coordinator creates typed presentation requests and selects at most one presenter. System delivery prefers the oldest eligible native presenter, then a foreground thin client. Terminal delivery uses the foreground thin client. Interactive App mode has no API foreground-client transport for window titles, so its title methods return `no_foreground_client`.
...
| `position` | `ToastGardnPosition \| null` | `null`   | Optional built-in toast position: `top-left`, `top-right`, `bottom-left`, or `bottom-right`. It has no effect for terminal/system delivery.           |
| `sound`    | `NotificationSound`          | `"none"` | `"none"`, `"done"`, or `"request"`. The coordinator sends visual and sound work to one selected presenter.                                            |
...
| ---------------------- | ------- | ------------------------------------------------------------------------ |
| `queued`               | `true`  | Gardn stored the toast or a presenter queue accepted the request.        |
| `suppressed`           | `false` | `toast.delivery` is `off`.                                               |
| `busy`                 | `false` | Built-in Gardn delivery already has a toast.                             |
| `rate_limited`         | `false` | Another API notification was queued less than 250 ms ago.                |
| `no_foreground_client` | `false` | No eligible presenter accepted terminal, system, or sound-only delivery. |

Built-in `gardn` delivery checks `busy` before the rate limit and stores one toast in App state. The coordinator sends its optional sound to one presenter. `terminal` and `system` delivery rate-limit first and then enqueue one typed presentation request. A queued result does not mean that a person saw the notification.
...
{"id":"notify-1","result":{"type":"notification_show","shown":true,"reason":"queued"}}
```

### `notification.presenter.register`

Registers a native presenter on a persistent JSON-lines connection. Params are `PresenterRegistration`: `name`, `rendering_host_id`, and boolean `terminal`, `system`, and `sound` capabilities. The first response returns `NotificationPresenterRegistered` with `registration_id`. The presenter becomes eligible only after Gardn writes this response.

Gardn writes later `PresentationRequest` objects to the same downstream stream. Each request contains the registration ID and one typed state notification. Closing the stream unregisters the presenter. Gardn does not replay pending requests after reconnect.

### `notification.presenter.receipt`

Records a `PresentationReceipt` on a separate Local API connection. Params contain `registration_id`, `notification_id`, and `outcome`. Outcome is `"submitted"`, `"unknown"`, or `{"rejected":"reason"}`. The result reports `accepted: true` only when both IDs and the owning presenter match one outstanding request. A receipt never triggers fallback or replay.
...
The `IntegrationTarget` values are `pi`, `omp`, `claude`, `codex`, `copilot`, `devin`, `kimi`, `droid`, `opencode`, `hermes`, `qodercli`, `qwen`, `kilo`, `mastracode`, `antigravity_cli`, `cursor`, `grok`, and `amp`.

All targets support macOS and Linux. Native Windows supports every target except `amp`. Amp supports WSL.

The installer checks the agent version for `kimi` and `amp`. Amp requires version `0.0.1789716701` or newer. An old version fails installation. An unavailable or unparsable version adds a warning and installation continues.
...
| ----------------- | ---------------------------------------------------------------------------------------------- |
| `pi`              | Pi extension.                                                                                  |
| `omp`             | One or more Gardn extensions.                                                                  |
...
| `droid`           | Droid hook and hooks entry.                                                                    |
| `opencode`        | OpenCode plugin and TUI session plugin, with the TUI session plugin enabled in the TUI config. |
| `hermes`          | Hermes plugin directory and enabled config entry.                                              |
| `qodercli`        | Qoder CLI hook and settings entry.                                                             |
| `qwen`            | Qwen Code hook and settings entry.                                                             |
| `kilo`            | Kilo JavaScript plugin.                                                                        |
| `mastracode`      | MastraCode hook and hooks entry.                                                               |
| `antigravity_cli` | Antigravity CLI hook and hooks entry.                                                          |
| `cursor`          | Cursor hook and hooks entry.                                                                   |
| `grok`            | Grok hook and hook config.                                                                     |
| `amp`             | Amp TypeScript plugin.                                                                         |

The Amp plugin is installed at `$XDG_CONFIG_HOME/amp/plugins/gardn-agent-state.ts`. If `XDG_CONFIG_HOME` is unset or empty, the path is `~/.config/amp/plugins/gardn-agent-state.ts`.
...
A manifest is loaded from `gardn-plugin.toml` or a Herdr v0.8.2 compatibility manifest named `herdr-plugin.toml`. Gardn manifests require `min_gardn_version`; Herdr manifests require `min_herdr_version`. Plugin IDs are trimmed, non-empty, at most 120 characters, and contain only ASCII letters, digits, `:`, `.`, `_`, and `-`. Local action, pane, and link-handler IDs are trimmed, non-empty, at most 120 characters, and contain only ASCII letters, digits, `:`, `_`, and `-` (no dot). Platform values are `linux`, `macos`, and `windows`.

An installed plugin record contains `plugin_id`, `name`, `version`, `min_gardn_version`, `manifest_dialect`, optional `description`, `manifest_path`, `plugin_root`, `enabled`, optional plugin `platforms`, arrays `build`, `startup`, `actions`, `events`, `panes`, and `link_handlers`, `source`, and `warnings`. A missing source manifest is retained with a warning beginning `manifest unavailable: `; action listing skips it and action/pane execution rejects it.
...
Manifest validation also requires non-empty `name`, `version`, and the dialect-specific semantic minimum version, non-empty command argv, valid platform names, unique action/pane/link-handler IDs, valid link-handler regexes, and link handlers that reference an existing action. Popup dimensions are valid only for popup placement. Unknown event names are retained as warnings; only known hook event kinds execute.
...
| `invalid_plugin_id`, `invalid_plugin_name`, `invalid_plugin_version`, `invalid_plugin_min_gardn_version`, `invalid_plugin_min_herdr_version` | A required identity/version field is invalid.                            |
| `plugin_requires_newer_gardn`, `plugin_requires_newer_herdr`                                                                                 | The manifest requires an unsupported newer runtime.                      |
...
Opens a pane declared by a plugin manifest. The process runs as the current user. Split, tab, and zoomed placements create ordinary session panes; overlay and popup placement create a detached popup runtime owned by the requesting client view. The server records plugin attribution and emits `pane.created` (and `tab.created` for `tab` placement) before returning the success response.
...
| `placement`      | `PluginPanePlacement \| null` | manifest placement, otherwise `overlay` | `overlay`, `popup`, `split`, `tab`, or `zoomed`.                                                           |
| `width`          | `PopupSize \| null`           | manifest popup width                    | Popup outer width in cells or a percentage such as `"80%"`. Valid only for `popup`.                        |
| `height`         | `PopupSize \| null`           | manifest popup height                   | Popup outer height in cells or a percentage such as `"80%"`. Valid only for `popup`.                       |
...
| `popup`   | Rejects `workspace_id`, `target_pane_id`, and `direction`; opens a client-owned detached popup and accepts `width` and `height`.                    |
...
The plugin manifest's pane platform list overrides its plugin platform list. The launch environment removes caller attempts to set protected `GARDN_*` and `HERDR_*` identity, path, context, executable, socket, and environment keys, then replaces them with server values.

Only the client view that opened an overlay or popup can see, focus, or close it through these methods. A second request from that client replaces its current popup. The popup closes and its runtime and plugin attribution are removed when the owner closes it, disconnects, or the popup process exits. Other attached clients receive `plugin_pane_not_found` for that popup id. These ownership rules do not apply to split, tab, or zoomed plugin panes.
