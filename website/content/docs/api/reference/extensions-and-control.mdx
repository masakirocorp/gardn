...
| `droid`    | Droid hook and hooks entry; legacy Gardn Droid entries may also be removed.              |
...
A manifest is loaded from `gardn-plugin.toml`; when a directory has no such file, the legacy `gardn-plugin.toml` filename is tried. Plugin IDs are trimmed, non-empty, at most 120 characters, and contain only ASCII letters, digits, `:`, `.`, `_`, and `-`. Local action, pane, and link-handler IDs are trimmed, non-empty, at most 120 characters, and contain only ASCII letters, digits, `:`, `_`, and `-` (no dot). Platform values are `linux`, `macos`, and `windows`.
...
Manifest validation also requires non-empty `name`, `version`, and semantic `min_gardn_version` (or compatible legacy `min_gardn_version`), non-empty command argv, valid platform names, unique action/pane/link-handler IDs, valid link-handler regexes, and link handlers that reference an existing action. Unknown event names are retained as warnings; only known hook event kinds execute.
...
| `invalid_plugin_id`, `invalid_plugin_name`, `invalid_plugin_version`, `invalid_plugin_min_gardn_version`                                   | A required identity/version field is invalid.                                |
| `plugin_requires_newer_gardn`                                                                                                              | The manifest requires a newer Gardn or incompatible legacy plugin API. |
