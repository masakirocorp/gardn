...
Closes a pane attributed to a plugin and removes its attribution. The underlying standard pane-close path runs, so it can also return standard close errors such as `confirmation_required` when the configured close policy requires confirmation.
