...
- `ping`.
- `connection.list`, `connection.save`, `connection.delete`, `connection.install`, `connection.test`, `connection.connect`, and `connection.disconnect`.
...
Removes a profile by `profile_id`. Returns `ConnectionDeleted { profile_id, removed }`; `removed` is `false` when the id did not exist.

Errors: `connection_profile_delete_failed`.

### `connection.install`

Previews or applies the remote execution-worker install/update for a saved profile. Call this after `connection.save` and before relying on remote placement or `connection.connect` on a clean host.
...
Without confirmation the remote host is not mutated. Confirmed installs may complete asynchronously; clients must handle the deferred install disposition the same way as other long-running connection work.
