...
Reference for the extension and control methods in the Local API: server control, notifications, window titles, integrations, plugins, configuration diagnostics, and event subscriptions. Workspace, tab, agent, and pane methods are documented on [Resources](/docs/api/reference/resources) and [Panes and agents](/docs/api/reference/panes-and-agents). For the canonical wire shapes see the [generated reference](/docs/api/reference/generated). For the error envelope see [Error semantics](/docs/api/errors). Transport and the request envelope are described in the [Local API overview](/docs/api).
