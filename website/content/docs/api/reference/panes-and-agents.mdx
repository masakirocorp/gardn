...
This is the authored reference for the `agent.*` and `pane.*` Local API request methods. It documents behavior that the generated schema cannot express: targeting rules, input and read semantics, layout identity consequences, and exact success discriminators.

For the raw parameter and result shapes, see the generated reference:

- [Request methods](/docs/api/reference/generated/requests)
- [Successful responses](/docs/api/reference/generated/responses)
- [Enum values](/docs/api/reference/generated/enums) (`PaneDirection`, `SplitDirection`, `ReadSource`, `ReadFormat`, `PaneAgentState`, `PaneZoomMode`, `PaneMoveReason`, `PaneSwapReason`, `PaneResizeReason`, `PaneZoomReason`, `PaneFocusDirectionReason`)
- [Error semantics](/docs/api/errors)

Transport, request envelope, trust model, and the one-request/one-response lifecycle are described in the [Local API overview](/docs/api).

Unless a method says otherwise, it follows the standard response lifecycle: one newline-delimited JSON request, one newline-delimited JSON response that echoes the caller-supplied `id`. `pane.wait_for_output` and `events.subscribe` keep the connection open; see their sections.
...
`agent.get`, `agent.focus`, `agent.read`, `agent.send`, `agent.rename`, `agent.explain` accept a free-form `target` string resolved in this order:
...
Every mutating pane method returns a result object with a `changed` boolean. Treat `changed` as the authoritative success discriminator for whether state was actually modified:
...
| `agent_not_found` | `agent.get`, `agent.focus`, `agent.read`, `agent.send`, `agent.rename`, `agent.explain`. | `target` did not resolve to any agent terminal. |
...
| `confirmation_required` | `pane.close`. | Closing the pane would also close a worktree group the server will not implicitly remove. |
| `pane_send_failed` / `agent_send_failed` | `pane.send_text`, `pane.send_keys`, `pane.send_input`, `agent.send`. | The runtime writer reported an error. |
| `invalid_key` | `pane.send_keys`, `pane.send_input`. | A requested key could not be parsed. The message is `unsupported key <key>`. |
| `invalid_regex` | `pane.wait_for_output`. | A `regex` match pattern failed to compile. |
| `timeout` | `pane.wait_for_output`. | The match did not occur before the caller-supplied deadline. See [pane.wait_for_output](#panewait_for_output). |
...
```json
{"id":"ex:agent-list","result":{"type":"agent_list","agents":[{"terminal_id":"term_1943cd2b11a1","name":"builder","agent":"claude","agent_status":"working","workspace_id":"w1943cd2b11a1","tab_id":"w1943cd2b11a1:t1","pane_id":"w1943cd2b11a1:p2","focused":false,"revision":12}]}}
```
...
| `cwd` | `string \| null` | no | process cwd (fallback `/`) | Working directory for the new terminal. |
...
Errors: `invalid_agent_name`, `invalid_agent_argv`, `agent_name_taken`, `agent_placement_not_found`, `agent_placement_conflict`, `agent_start_failed`, `invalid_env`.

```json
{"id":"ex:start","method":"agent.start","params":{"name":"builder","argv":["claude","--no-tips"],"cwd":"/repo","split":"down","focus":true,"env":{"CLAUDE_BUDGET":"200"}}}
```

```json
{"id":"ex:start","result":{"type":"agent_started","agent":{"terminal_id":"term_1943cd2b22b2","name":"builder","agent_status":"unknown","screen_detection_skipped":false,"workspace_id":"w1943cd2b11a1","tab_id":"w1943cd2b11a1:t1","pane_id":"w1943cd2b11a1:p3","focused":true,"revision":0},"argv":["claude","--no-tips"]}}
```
...
- `revision` is always `0` and `truncated` is always `false` in the current implementation. These fields exist for forward compatibility; do not treat them as a content revision.
...
Write raw bytes from a string to the terminal behind an agent target. The bytes are written as-is without key parsing and without bracketed paste wrapping. This is the lowest-level send primitive and is appropriate for streaming agent input that should not be reparsed.
...
Result: `Ok {}`. A failed write returns `agent_send_failed` with the runtime error message.

Errors: `agent_not_found`, `agent_target_ambiguous`, `agent_send_failed`.

```json
{"id":"ex:send","method":"agent.send","params":{"target":"builder","text":"summarize the failing tests and propose a fix\n"}}
```

```json
{"id":"ex:send","result":{"type":"ok"}}
```
...
Result: `PaneRead { read: PaneReadResult }` with `pane_id` echoed, `workspace_id`, `tab_id`, `source`, `format`, `text`, `revision`, and `truncated`. The current `pane.read` handler always returns `revision: 0` and `truncated: false`; do not depend on them for change detection.

Errors: `pane_not_found`.

```json
{"id":"ex:read","method":"pane.read","params":{"pane_id":"w1943cd2b11a1:p2","source":"recent","lines":40,"format":"text"}}
```

```json
{"id":"ex:read","result":{"type":"pane_read","read":{"pane_id":"w1943cd2b11a1:p2","workspace_id":"w1943cd2b11a1","tab_id":"w1943cd2b11a1:t1","source":"recent","format":"text","text":"$ ls\nsrc docs\n","revision":0,"truncated":false}}}
```
...
```json
{"id":"ex:wait","method":"pane.wait_for_output","params":{"pane_id":"w1943cd2b11a1:p2","source":"recent","match":{"type":"substring","value":"Tests: 0 failed"},"timeout_ms":30000}}
```

```json
{"id":"ex:wait","result":{"type":"output_matched","pane_id":"w1943cd2b11a1:p2","revision":0,"matched_line":"Tests: 0 failed in 3.4s","read":{"pane_id":"w1943cd2b11a1:p2","workspace_id":"w1943cd2b11a1","tab_id":"w1943cd2b11a1:t1","source":"recent_unwrapped","format":"text","text":"$ cargo test\nTests: 0 failed in 3.4s\n","revision":0,"truncated":false}}}
```
...
Return the `PaneInfo` for the current pane. The optional `caller_pane_id` overrides resolution: when present, the server parses and returns that pane instead of the active one. When the resolved pane does not exist, the error is `pane_not_found` with the generic message `pane not found`.
...
| `cwd` | `string \| null` | no | resolved from the source pane's effective cwd when available | Working directory for the new pane. |
| `env` | `object` | no | `{}` | Extra environment variables. See [env behavior](#env-behavior). |
...
Errors: `pane_not_found`, `invalid_env`, `pane_split_failed`.

```json
{"id":"ex:split","method":"pane.split","params":{"direction":"down","ratio":0.4,"cwd":"/repo","focus":true}}
```

```json
{"id":"ex:split","result":{"type":"pane_info","pane":{"pane_id":"w1943cd2b11a1:p4","terminal_id":"term_1943cd2b33c3","workspace_id":"w1943cd2b11a1","tab_id":"w1943cd2b11a1:t1","focused":true,"agent_status":"idle","revision":0}}}
```

### pane.close

Close a pane. When the pane is the last pane in its tab, the tab closes. When the pane is the last pane in its workspace and the workspace is a worktree group the server protects from implicit close, the call returns `confirmation_required`.
...
Move a pane to a destination: an existing tab (split into it), a new tab in an existing workspace, or a new workspace. This is the most identity-affecting pane method.
...
- The moved pane keeps its underlying pane id and terminal id across workspace boundaries, but the public `pane_id` changes when the move crosses workspaces. The previous public id is registered as an alias of the new id; `parse_pane_id` with the previous id still resolves to the moved pane.
...
```json
{"id":"ex:move","method":"pane.move","params":{"pane_id":"w1943cd2b22b2:p1","destination":{"type":"tab","tab_id":"w1943cd2b11a1:t1","target_pane_id":"w1943cd2b11a1:p1","split":"right","ratio":0.3},"focus":true}}
```

```json
{"id":"ex:move","result":{"type":"pane_move","move_result":{"changed":true,"reason":null,"previous_pane_id":"w1943cd2b22b2:p1","previous_workspace_id":"w1943cd2b22b2","previous_tab_id":"w1943cd2b22b2:t1","pane":{"pane_id":"w1943cd2b11a1:p2","terminal_id":"term_1943cd2b44d4","workspace_id":"w1943cd2b11a1","tab_id":"w1943cd2b11a1:t1","focused":true,"agent_status":"idle","revision":0},"source_layout":null,"target_layout":{"workspace_id":"w1943cd2b11a1","tab_id":"w1943cd2b11a1:t1","zoomed":false,"focused_pane_id":"w1943cd2b11a1:p2","panes":[]},"created_workspace":null,"created_tab":null,"closed_workspace_id":"w1943cd2b22b2","closed_tab_id":"w1943cd2b22b2:t1","focused_pane_id":"w1943cd2b11a1:p2"}}}
```
...
- `foreground_processes`: the processes in the detected foreground job. Each entry has `pid`, `name`, optional `argv0`, optional `argv`, optional `cmdline`, and optional `cwd` (resolved via `process_cwd`; absence is not an error). Empty when no foreground job is detected.
...
- The split ratio does not actually change after clamping (e.g., already at `0.1`/`0.9` bounds): `changed: false`, `reason: "unchanged"`.
...
Report whether the resolved pane touches each edge of the terminal area. This is a read-side companion to `pane.neighbor`: it tells you a pane is on the border without naming a neighbor.
...
```json
{"id":"layout:apply","method":"layout.apply","params":{"workspace_id":"w1943cd2b11a1","tab_label":"review","focus":true,"root":{"type":"split","direction":"right","ratio":0.65,"first":{"type":"pane","label":"editor","cwd":"/repo"},"second":{"type":"pane","label":"tests","cwd":"/repo","command":["sh","-lc","just test"]}}}}
```
...
| --- | --- | --- | --- |
| `pane_id` | `string` | yes | — |
| `source` | `string` | yes | — | Free-form, conventionally `omh:<agent>` or `<integration>:<agent>`. Used by the resume subsystem to correlate reports. |
| `agent` | `string` | yes | — | Agent label; trimmed, normalized through `parse_agent_label` when known, kept verbatim otherwise. Empty produces `invalid_agent`. |
...
```json
{"id":"ex:report-agent","method":"pane.report_agent","params":{"pane_id":"w1943cd2b11a1:p2","source":"omh:claude","agent":"claude","state":"blocked","message":"awaiting human review of migration plan","custom_status":"needs decision","agent_session_id":"sess-abc","seq":7}}
```
...
| `agent` | `string \| null` | no | `null` | Optional agent label; normalized through `normalize_reported_agent_label`. Empty produces `invalid_agent`. |
...
```json
{"id":"ex:report","method":"pane.report_metadata","params":{"pane_id":"w1943cd2b11a1:p2","source":"omh:claude","title":"migration review","custom_status":"awaiting input","state_labels":{"blocked":"needs decision"},"tokens":{"branch":"migrate-2","pr":"1247"},"seq":42}}
```
...
`agent.start` and `pane.split` accept an `env` object passed through `normalize_launch_env`:
...
| [`agent.send`](#agentsend) | `AgentSendParams` | `Ok` |
