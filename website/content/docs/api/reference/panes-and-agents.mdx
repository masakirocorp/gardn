...
`agents` contains one `AgentInfo` per pane whose terminal is marked as an agent terminal, in workspace then tab then traversal order. Empty when no agents exist. `follow_up` and `follow_up_added_at_unix_secs` come from the same Follow Up queue the Agents sidebar uses. `last_meaningful_agent_activity_unix_secs` is the timestamp that sidebar uses for age.
    ...
    "agents": [
      {
        ...
        "revision": 12,
        "follow_up": true,
        "follow_up_added_at_unix_secs": 1700000000
      }
    ]
  }
}
