...
Reference for the `agent.*`, `pane.*`, and `layout.*` Local API methods: targeting rules, input and read semantics, layout identity consequences, and success discriminators. For the raw parameter and result shapes see the generated [requests](/docs/api/reference/generated/requests), [responses](/docs/api/reference/generated/responses), and [enums](/docs/api/reference/generated/enums) (`PaneDirection`, `SplitDirection`, `ReadSource`, `ReadFormat`, `PaneAgentState`, `PaneZoomMode`, `PaneMoveReason`, `PaneSwapReason`, `PaneResizeReason`, `PaneZoomReason`, `PaneFocusDirectionReason`). For the error envelope see [Error semantics](/docs/api/errors). Transport and the request envelope are described in the [Local API overview](/docs/api).

`pane.wait_for_output` is the one method on this page that holds its connection open; see its entry.
