...
description: Connect to an Gardn Unix socket and correlate one JSON response.
---
...
Get the socket path from `gardn status --json` and provide it to the script as `GARDN_SOCKET_PATH`.
...
The result identifies the running Gardn version, the interactive client protocol version, and any advertised server capabilities.
