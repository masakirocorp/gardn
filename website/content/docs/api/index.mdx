...
The Local API is the lowest-level public automation surface. It is not automatically safer or more stable than the CLI: callers own socket selection, framing, timeouts, retries, and reconciliation.
...
For a first integration, continue with [Send a request from a local script](/docs/api/workflow). [Error semantics](/docs/api/errors) explains transport failures, API failures, retries, and streaming recovery.
