...
This reference describes the built-in v2 keybindings. The default prefix is `ctrl+b`: press it first, then the listed key. A blank default is unset; it does not mean that an alternate shortcut is guaranteed. Use the [workspaces and navigation guide](/docs/guides/workspaces-and-navigation) for task-oriented examples.
