...
| `github`  | string | `ghui`              | Mouse-aware GitHub pull request and issue client. |

**Settings > Commands** edits these values. Clear one field to hide that project action. Each field can restore its built-in value, and **Reset All Commands** restores all four defaults. The curated ghui command requires Gardn's pinned Masakiro companion release. It starts in the local Space directory and receives launch-only theme, scrollbar, Space name, repository scope, and verified checkout paths. **Space Settings > GitHub** stores Automatic, Selected repositories, or Group organization scope. A running ghui retains its launch scope until restarted.
