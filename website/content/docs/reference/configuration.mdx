...
**Settings > Commands** edits these values. Clear one field to hide that project action. Each field can restore its built-in value, and **Reset All Commands** restores all four defaults. The curated ghui command starts in the workspace directory and uses an isolated runtime theme configuration.
