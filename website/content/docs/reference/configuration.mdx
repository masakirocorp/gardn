...
| `github`  | string | `gh dash`           | GitHub pull request and issue dashboard.      |

**Settings > Commands** edits these values. Clear one field to hide that project action. Each field can restore its built-in value, and **Reset All Commands** restores all four defaults. Group settings can optionally provide a validated GitHub organization; workspaces inherit that setting when launching GitHub.
