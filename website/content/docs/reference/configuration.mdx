...
An installed release of `Gardn.app` sets `delivery = "system"` when neither `ui.toast.delivery` nor the legacy `ui.toast.enabled` key exists. It preserves every explicit notification choice.
...
Amp uses the system profile `system:amp` and launches `amp`. Use `kind = "amp"` for a custom Amp wrapper. Its installed integration reports the selected thread for native restore.
