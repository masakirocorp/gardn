...
**Settings > Commands** edits these values. Clear one field to hide that project action. Each field can restore its built-in value, and **Reset All Commands** restores all four defaults. The curated ghui command requires Gardn's pinned Masakiro companion release. It starts in the workspace directory and receives launch-only theme, scrollbar, and optional Group GitHub organization settings.
