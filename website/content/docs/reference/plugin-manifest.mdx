...
Gardn also accepts a Herdr v0.8.2 manifest named `herdr-plugin.toml`. It uses `min_herdr_version` instead of `min_gardn_version`. Both compatibility keys are required by their respective dialect and may not require a version newer than their supported runtime.
...
- `description` is optional.
- `platforms` overrides the plugin-level list when present.
- `placement` defaults to `overlay` and may be `overlay`, `popup`, `split`, `tab`, or `zoomed`.
- `width` and `height` are optional only for `popup`. Each is an outer popup size in terminal cells or a percentage string such as `"80%"`.
- `command` is the direct argv command for the pane process.

The pane command's default working directory is the plugin root. A pane-open request can select a working-directory override, placement, popup dimensions, focus behavior, split target/direction, workspace, and additional environment entries. Gardn rejects placement parameters that do not apply to the selected placement.
...
Herdr v0.8.2 manifests receive equivalent protected `HERDR_*` names. These aliases include `HERDR_ENV`, `HERDR_SOCKET_PATH`, plugin path and identity values, invocation context, executable path, and applicable action, event, workspace, tab, and pane values. Plugin pane environment overrides cannot replace either protected Gardn or Herdr keys.
