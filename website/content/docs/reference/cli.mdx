...
| `omh agent start <name> [--cwd PATH] [--workspace ID] [--tab ID] [--split right\|down] [--focus\|--no-focus] -- <argv...>` | Start a named agent process in a pane. The arguments after `--` form the exact process argument vector; no shell parsing is performed. |
