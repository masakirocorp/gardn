...
| `omh connection delete <profile_id>`                                                  | Delete a profile.                                                                   |
| `omh connection install <profile_id> [--yes]`                                         | Preview the worker install or update plan; with `--yes`, apply that confirmed plan. |
| `omh connection test <profile_id>`                                                    | Queue an OpenSSH connectivity test.                                                 |
| `omh connection connect <profile_id>`                                                 | Queue a connection request.                                                         |
| `omh connection disconnect <profile_id>`                                              | Queue a disconnect request.                                                         |

Install the worker before `connect` on a clean host. Without `--yes`, `connection install` returns the preview only. Updates use the same confirmation path. The lifecycle commands return when the action is queued; run `omh connection list` to observe the resulting status.
