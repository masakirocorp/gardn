...
def commit_subjects(previous: str, commit: str) -> list[str]:
    ...
    for line in output.splitlines():
        stripped = line.strip()
        if stripped and not hidden_subject(stripped):
...
def humanize_subject(subject: str) -> tuple[str, str]:
    ...
    return heading, body[0].upper() + body[1:] if body else subject
...
def build_manifest(
    ...
    assets = asset_objects(default_asset_urls(repo, tag), shas)
...
def main() -> int:
    ...
    notes.add_argument("--manifest", default="preview.json")
    ...
    notes.add_argument("--repo", default="masakirocorp/hako")
    notes.add_argument("--output", required=True)
    notes.set_defaults(func=cmd_notes)

    manifest = sub.add_parser("manifest")
    manifest.add_argument("--output", default="preview.json")
    manifest.add_argument("--repo", default="masakirocorp/hako")
    ...
    current.add_argument("--manifest", default="preview.json")
