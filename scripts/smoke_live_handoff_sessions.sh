...
GARDN_BIN="${GARDN_BIN:-$ROOT/target/debug/gardn}"
BASE="${BASE:-$(mktemp -d /tmp/gardn-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_gardn "$session" server stop >/dev/null 2>&1 || true
  done
...
run_gardn() {
  ...
  env -u GARDN_SOCKET_PATH \
    -u GARDN_CLIENT_SOCKET_PATH \
    -u GARDN_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$GARDN_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/gardn-dev' "$CONFIG_HOME"
  else
    printf '%s/gardn-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/gardn.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/gardn-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/gardn-dev/gardn.sock | "$CONFIG_HOME"/gardn-dev/sessions/*/gardn.sock)
      ;;
    *)
...
echo "using gardn: $GARDN_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/apps/gardn/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/gardn-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/gardn-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_gardn "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
