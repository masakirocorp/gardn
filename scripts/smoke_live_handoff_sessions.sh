...
OMH_BIN="${OMH_BIN:-$ROOT/target/debug/omh}"
BASE="${BASE:-$(mktemp -d /tmp/omh-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_omh "$session" server stop >/dev/null 2>&1 || true
  done
...
run_omh() {
  ...
  env -u OMH_SOCKET_PATH \
    -u OMH_CLIENT_SOCKET_PATH \
    -u OMH_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$OMH_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/omh-dev' "$CONFIG_HOME"
  else
    printf '%s/omh-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/omh.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/omh-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/omh-dev/omh.sock | "$CONFIG_HOME"/omh-dev/sessions/*/omh.sock)
      ;;
    *)
...
echo "using omh: $OMH_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/apps/omh/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/omh-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/omh-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_omh "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
