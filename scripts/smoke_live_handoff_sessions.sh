...
HAKO_BIN="${HAKO_BIN:-$ROOT/target/debug/hako}"
BASE="${BASE:-$(mktemp -d /tmp/hako-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_hako "$session" server stop >/dev/null 2>&1 || true
  done
...
run_hako() {
  ...
  env -u HAKO_SOCKET_PATH \
    -u HAKO_CLIENT_SOCKET_PATH \
    -u HAKO_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$HAKO_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/hako-dev' "$CONFIG_HOME"
  else
    printf '%s/hako-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/hako.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/hako-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/hako-dev/hako.sock | "$CONFIG_HOME"/hako-dev/sessions/*/hako.sock)
      ;;
    *)
...
for session in "${sessions[@]}"; do
  ...
  run_hako "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
