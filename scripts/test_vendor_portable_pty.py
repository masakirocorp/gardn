...
class VendorPortablePtyTests(unittest.TestCase):
    def test_vendored_tree_contains_required_upstream_files(self) -> None:
        root = Path(__file__).resolve().parent.parent / "apps" / "hako" / "vendor" / "portable-pty"
        required = [
            root / "Cargo.toml",
            root / "LICENSE.md",
            root / "src" / "lib.rs",
    ...
    def test_cargo_patch_points_at_vendored_tree(self) -> None:
        ...
        app_manifest = (project_root / "apps" / "hako" / "Cargo.toml").read_text()

        self.assertIn('portable-pty = "=0.9.0"', app_manifest)
        self.assertIn("[patch.crates-io]", cargo_toml)
        self.assertIn(
            'portable-pty = { path = "apps/hako/vendor/portable-pty" }',
            cargo_toml,
        )
    ...
    def test_nix_source_includes_vendored_tree(self) -> None:
        ...
        self.assertIn("../apps/hako/vendor/portable-pty", package_nix)
    ...
    def test_cargo_metadata_resolves_portable_pty_to_vendored_tree(self) -> None:
        ...
        expected = (
            project_root / "apps" / "hako" / "vendor" / "portable-pty" / "Cargo.toml"
        ).resolve()
        self.assertEqual(manifest_path, expected)

    def test_local_vendor_patches_are_listed_in_patch_index(self) -> None:
        project_root = Path(__file__).resolve().parent.parent
        index = project_root / "apps" / "hako" / "vendor" / "portable-pty.patches.md"
        patch_dir = project_root / "apps" / "hako" / "vendor" / "patches" / "portable-pty"
