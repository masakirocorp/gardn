...
class VendorLibghosttyVtTests(unittest.TestCase):
    ...
    def test_vendored_tree_contains_required_upstream_files(self) -> None:
        root = Path(__file__).resolve().parent.parent / "apps" / "hako" / "vendor" / "libghostty-vt"
        required = [
            root / "build.zig",
            root / "build.zig.zon",
            root / "CMakeLists.txt",
    ...
    def test_vendor_metadata_exists_and_points_at_vendored_tree(self) -> None:
        ...
        metadata = project_root / "apps" / "hako" / "vendor" / "libghostty-vt.vendor.json"
    ...
    def test_local_vendor_patches_are_listed_in_patch_index(self) -> None:
        project_root = Path(__file__).resolve().parent.parent
        index = project_root / "apps" / "hako" / "vendor" / "libghostty-vt.patches.md"
        patch_dir = project_root / "apps" / "hako" / "vendor" / "patches" / "libghostty-vt"
