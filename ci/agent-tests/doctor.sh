...
bins=(claude codex opencode copilot hermes droid kimi maki qwen kilo kilo-code mastracode amp agy cursor-agent qoder qodercli omp pi jq python3 node pnpm git)
missing=0
for bin in "${bins[@]}"; do
...
for bin in claude codex opencode copilot hermes droid kimi maki qwen kilo kilo-code mastracode amp agy cursor-agent qoder qodercli omp pi; do
  printf '%s: ' "$bin"
