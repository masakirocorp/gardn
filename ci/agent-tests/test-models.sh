...
hako_test_unique_candidates() {
  local primary="$1"
  local fallbacks="${2:-${HAKO_TEST_FALLBACK_MODELS:-}}"
...
hako_test_openrouter_bare_candidates() {
  while IFS= read -r model; do
    case "$model" in
      openrouter/*) printf '%s\n' "${model#openrouter/}" ;;
      *) printf '%s\n' "$model" ;;
...
hako_test_retryable_status_or_output() {
  local status="$1"
  local output="$2"
  [[ "$status" -eq 124 ]] || hako_test_retryable_output "$output"
}

hako_test_run_with_fallbacks() {
  local script="$1"
  local env_name="$2"
  shift 2
  local attempts=()
  local model status
  local candidates=()
  if [[ -n "${HAKO_TEST_ACTIVE_MODEL:-}" ]]; then
    return 0
  fi
  while IFS= read -r model; do
  ...
  for model in "${candidates[@]}"; do
    ...
    set +e
    HAKO_TEST_ACTIVE_MODEL="$model" "$script" "$@" >"$attempt_output" 2>&1
    status=$?
    set -e
    ...
    if [[ "$status" -eq 75 ]] || hako_test_retryable_status_or_output "$status" "$attempt_output"; then
  ...
  printf 'all test model candidates failed before Hako assertions:\n' >&2
  printf '  %s\n' "${attempts[@]}" >&2
  return 75
}
