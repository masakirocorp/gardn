...
const LOG = process.env.GARDN_CURSOR_PROXY_LOG || '/tmp/gardn-cursor-proxy.log';
const CERT = process.env.GARDN_CURSOR_PROXY_CERT;
const KEY = process.env.GARDN_CURSOR_PROXY_KEY;
const MODEL = process.env.GARDN_TEST_CURSOR_MODEL || process.env.GARDN_TEST_MODEL || 'openrouter/auto';
...
if (!CERT || !KEY) throw new Error('GARDN_CURSOR_PROXY_CERT and GARDN_CURSOR_PROXY_KEY are required');
...
server.on('request', (req, res) => {
  ...
  req.on('end', () => {
    ...
    if (url === '/auth/exchange_user_api_key') {
      ...
      res.end(JSON.stringify({accessToken:'gardn-test-token', refreshToken:'gardn-test-refresh', authId:'gardn-test'}));
      return;
    }
...
server.on('stream', (stream, headers) => {
  ...
  const handle = async (force = false) => {
    ...
    if (process.env.GARDN_CURSOR_PROXY_STATIC_REPLY) {
      stream.write(tdf(process.env.GARDN_CURSOR_PROXY_STATIC_REPLY));
...
function selectPrompt(strs) {
  ...
  return useful.sort((a, b) => b.length - a.length)[0]
    || 'Reply exactly GARDN_CURSOR_PROXY_OK';
}
...
async function callOpenRouter(prompt, stream) {
  ...
  if (!emitted) stream.write(tdf('GARDN_CURSOR_PROXY_OK'));
  stream.write(tef()); stream.end(ct());
