...
if (!pluginPath) {
  console.error("usage: opencode-plugin-status-test.mjs <gardn-agent-state.js>");
  process.exit(1);
}

async function runScenario(name, events, options = {}) {
  const tmp = await import("node:fs/promises").then((fs) =>
    fs.mkdtemp(path.join(os.tmpdir(), `gardn-opencode-${name}-`)),
  );
  const socketPath = path.join(tmp, "gardn.sock");
  const requests = [];
  ...
  process.env.GARDN_PANE_ID = `pane-${name}`;
  process.env.GARDN_OPENCODE_IDLE_REPORT_DELAY_MS = "5";
  process.env.OPENCODE_CONFIG = path.join(tmp, "opencode.json");

  const { GardnAgentStatePlugin } = await import(
    `${pathToFileURL(pluginPath).href}?${name}-${Date.now()}-${Math.random()}`,
  );
  const hooks = await GardnAgentStatePlugin();
...
function assertCommon(scenario) {
  ...
  for (const request of scenario.reports) {
    ...
    if (params.source !== "gardn:opencode") fail(scenario, "wrong source");
