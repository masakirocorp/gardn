...
case "$target" in
  ...
  mastracode)
    ...
    ;;
  amp)
    exec /opt/gardn-agent-tests/bun /usr/local/lib/gardn-amp-proxy-test.mjs openrouter /usr/local/bin/gardn-agent-tests-amp-status
    ;;
  amp-deterministic)
    exec /opt/gardn-agent-tests/bun /usr/local/lib/gardn-amp-proxy-test.mjs deterministic /usr/local/bin/gardn-agent-tests-amp-status
    ;;
  antigravity)
