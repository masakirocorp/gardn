...
Development builds now install SSH workers from matching local build cohorts instead of published release assets. Oh My Herdr verifies the staged worker's source identity, target, protocols, lifecycle, and capabilities before it publishes the worker manifest.
